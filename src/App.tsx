...
// HÀM TIỆN ÍCH HOISTING (Chống sập web)
const safeArray = (arr: any) => Array.isArray(arr) ? arr : [];
const safeObject = (obj: any) => (obj && typeof obj === 'object' && !Array.isArray(obj)) ? obj : {};
const parseLocal = (key: string, defaultVal: any) => { try { const s = localStorage.getItem(key); return s && s !== "undefined" && s !== "null" ? JSON.parse(s) : defaultVal; } catch(e) { return defaultVal; } };
const formatCategoryStr = (str: string) => { try { if (!str) return "Khác"; const t = String(str).trim(); return t ? t.charAt(0).toUpperCase() + t.slice(1).toLowerCase() : "Khác"; } catch(e){return "Khác"} };
const parseGift = (giftStr: string | null) => { try { if (!giftStr) return { cond: 0, text: "" }; const s = String(giftStr); if (s.includes(';;;')) { const parts = s.split(';;;'); return { cond: parseInt(parts[0]) || 1, text: parts[1] || "" } } return { cond: 1, text: s } } catch(e){return {cond:0, text:""}} };
const cleanName = (name: string) => { try { return name ? String(name).split(' [Lô')[0] : ''; } catch(e){return ''} };
const getActualPrice = (p: any) => { try { if (!p) return 0; let price = (p.promo_price && p.promo_price > 0) ? p.promo_price : p.sale_price; const currentHour = new Date().getHours(); if ((currentHour >= 20 || currentHour < 6) && (p.category === 'Đồ ăn liền' || p.category === 'Bánh Kẹo')) { price = price * 0.8; p.isHappyHour = true } else { p.isHappyHour = false } return Math.round(price) } catch(e){return 0} };
const getCustomerTier = (totalSpent = 0) => { try { if (totalSpent >= 500000000) return { name: "💎 KIM CƯƠNG", discountRate: 0.10, color: "#a855f7", bg: "#faf5ff", border: "#e9d5ff" }; if (totalSpent >= 200000000) return { name: "🥇 VÀNG", discountRate: 0.05, color: "#ca8a04", bg: "#fefce8", border: "#fef08a" }; if (totalSpent >= 50000000) return { name: "🥈 BẠC", discountRate: 0.02, color: "#475569", bg: "#f8fafc", border: "#cbd5e1" }; return { name: "🥉 ĐỒNG", discountRate: 0, color: "#b45309", bg: "#fffbeb", border: "#fde68a" } } catch(e){return {name: "🥉 ĐỒNG", discountRate: 0, color: "#b45309", bg: "#fffbeb", border: "#fde68a"}} };
...
function MainApp() {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(() => { try { return localStorage.getItem("mart_logged_in") === "true"; } catch(e){return false} });
  const [role, setRole] = useState(() => { try { return localStorage.getItem("mart_role") || "staff"; } catch(e){return "staff"} });
  const [shift, setShift] = useState(() => { try { return localStorage.getItem("mart_shift") || "Ca Sáng"; } catch(e){return "Ca Sáng"} });
  const [authUsername, setAuthUsername] = useState("");
  const [authPassword, setAuthPassword] = useState("");
  const [currentTime, setCurrentTime] = useState(new Date());
  const [startingCash, setStartingCash] = useState<number>(() => { try { return Number(localStorage.getItem("mart_starting_cash")) || 0; } catch(e){return 0} });
  const [adminPass, setAdminPass] = useState(() => { try { return localStorage.getItem("mart_admin_pass") || "haile88"; } catch(e){return "haile88"} });
  const [staffPass, setStaffPass] = useState(() => { try { return localStorage.getItem("mart_staff_pass") || "123"; } catch(e){return "123"} });
  const [bankBin, setBankBin] = useState(() => { try { return localStorage.getItem("mart_bank_bin") || "970422"; } catch(e){return "970422"} });
  const [bankAcc, setBankAcc] = useState(() => { try { return localStorage.getItem("mart_bank_acc") || "0680124181004"; } catch(e){return "0680124181004"} });
  const [bankNameStr, setBankNameStr] = useState(() => { try { return localStorage.getItem("mart_bank_name") || "LE HONG HAI"; } catch(e){return "LE HONG HAI"} });
  const [darkMode, setDarkMode] = useState(() => { try { return localStorage.getItem("mart_theme") === "dark"; } catch(e){return false} });
  ...
  const loadCloudData = async () => {
    try {
      ...
      if (rCust && rCust.data && rCust.data.length > 0) { setCustomers((prev: any) => { const updated = { ...safeObject(prev) }; rCust.data.forEach((c: any) => { updated[c.phone] = { ...updated[c.phone], ...c }; }); return updated; }); }
      if (rHist && rHist.data) { setHistory(prev => { const cloudIds = new Set(rHist.data.map(h => h.id)); const localOnly = safeArray(prev).filter(h => !cloudIds.has(h.id)); return [...localOnly, ...rHist.data].sort((a, b) => b.id - a.id); }); }
      if (rExp && rExp.data) { setExpenses(prev => { const cloudIds = new Set(rExp.data.map(e => e.id)); const localOnly = safeArray(prev).filter(e => !cloudIds.has(e.id)); return [...localOnly, ...rExp.data].sort((a, b) => b.id - a.id); }); }
      if (rSup && rSup.data) { setSuppliers(prev => { const cloudIds = new Set(rSup.data.map(s => s.id)); const localOnly = safeArray(prev).filter(s => !cloudIds.has(s.id)); return [...localOnly, ...rSup.data].sort((a, b) => b.id - a.id); }); }
      if (rAud && rAud.data) { setAuditLogs(prev => { const cloudIds = new Set(rAud.data.map(a => a.id)); const localOnly = safeArray(prev).filter(a => !cloudIds.has(a.id)); return [...localOnly, ...rAud.data].sort((a, b) => b.id - a.id); }); }
      if (rHold && rHold.data) { setHeldOrders(prev => { const cloudIds = new Set(rHold.data.map(o => o.id)); const localOnly = safeArray(prev).filter(o => !cloudIds.has(o.id)); return [...localOnly, ...rHold.data].sort((a, b) => b.id - a.id); }); }
      setSyncStatus('synced');
  ...
  const fetchProducts = async () => { try{const { data } = await supabase.from("products").select("*").order("created_at", { ascending: false }); if (data) setProducts(data)}catch(e){} };
    ...
    try {
      if (isLoggedIn) {
        ...
        if(supabase && typeof supabase.channel === 'function') {
           ...
           script.onload = () => { if((window as any).emailjs) (window as any).emailjs.init(EMAILJS_PUBLIC_KEY); }; document.head.appendChild(script);
  ...
  const currentShiftStats = useMemo(() => { 
    ...
    } catch(e) { return { startingCash: 0, cashIn: 0, cashOut: 0, transferIn: 0, transferOut: 0, expectedCash: 0, prof: 0, totalSales: 0, shiftExp: 0, rev: 0 } }
  ...
  const todayStats = useMemo(() => { 
    ...
    } catch(e) { return { rev: 0, cash: 0, transfer: 0, prof: 0, totalSales: 0, expenses: 0, netProfit: 0 } }
  ...
  const handleLogin = (e: React.FormEvent) => { 
    ...
    try { localStorage.setItem("mart_starting_cash", startingCash.toString()); } catch(e){}
    if (u === "admin" && p === "khoiphuc88") { setAdminPass("haile88"); try{localStorage.removeItem("mart_admin_pass"); localStorage.removeItem("mart_staff_pass");}catch(e){} setStaffPass("123"); setAuthPassword(""); alert("✅ MK gốc:\nAdmin: haile88\nNV: 123"); return } 
    if (u === "admin" && p === adminPass) { setIsLoggedIn(true); setRole("admin"); try{localStorage.setItem("mart_shift", shift); localStorage.setItem("mart_logged_in", "true"); localStorage.setItem("mart_role", "admin");}catch(e){} logAudit("ĐĂNG NHẬP", "Mở ca", { start_cash: startingCash, role: "admin" }) } 
    else if (u === "nhanvien" && p === staffPass) { setIsLoggedIn(true); setRole("staff"); try{localStorage.setItem("mart_shift", shift); localStorage.setItem("mart_logged_in", "true"); localStorage.setItem("mart_role", "staff");}catch(e){} logAudit("ĐĂNG NHẬP", "Mở ca", { start_cash: startingCash, role: "staff" }) } 
  ...
  const confirmHandover = () => { 
      ...
      try { localStorage.removeItem("mart_logged_in"); localStorage.removeItem("mart_role"); localStorage.removeItem("mart_starting_cash"); } catch(e){}
      setStartingCash(0);
  };
  ...
  const saveSettings = () => { if (!newAdminPass || !newStaffPass || !newBankBin || !newBankAcc || !newBankNameStr) return alert("Điền đủ!"); setAdminPass(newAdminPass); try{localStorage.setItem("mart_admin_pass", newAdminPass); setStaffPass(newStaffPass); localStorage.setItem("mart_staff_pass", newStaffPass); setBankBin(newBankBin); localStorage.setItem("mart_bank_bin", newBankBin); setBankAcc(newBankAcc); localStorage.setItem("mart_bank_acc", newBankAcc); setBankNameStr(newBankNameStr); localStorage.setItem("mart_bank_name", newBankNameStr);}catch(e){} logAudit("CÀI ĐẶT", "Cập nhật Cấu hình"); alert("✅ Đã lưu!"); setShowSettings(false) };
    ...
    if (this.state.hasError) {
      return (
        <div style={{ padding: "40px", background: "#fef2f2", color: "#b91c1c", minHeight: "100vh", fontFamily: "sans-serif" }}>
          ...
          <p>Dữ liệu cũ trong trình duyệt của bạn đang bị xung đột với phiên bản mới.</p>
...
// XUẤT RA 2 TÊN ĐỂ CHỐNG LỖI INDEX.TSX TRÊN VERCEL
