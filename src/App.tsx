...
export default function App() {
  ...
  // Gọi API Key từ két sắt bảo mật của Vercel
  ...
  // =====================================================================
  // 3. ACTION FUNCTIONS (HÀM XỬ LÝ) - BAO GỒM CÁC HÀM ĐƯỢC BỔ SUNG
  // =====================================================================
  ...
  // --- BỔ SUNG CÁC HÀM XỬ LÝ BỊ THIẾU ---
  ...
  const handleNextToQR = () => setCheckoutStep('QR');
  
  const confirmCheckout = (method: string) => {
    if (cart.length === 0) return;
    const finalTotalToPay = Math.max(0, cart.reduce((sum, item) => sum + item.total, 0) - appliedVoucherAmount - (custPhone ? Math.round(cart.reduce((sum, item) => sum + item.total, 0) * getCustomerTier(customers[custPhone]?.totalSpent || 0).discountRate) : 0) - (useWallet ? Math.min(customers[custPhone]?.wallet || 0, Math.max(0, cart.reduce((sum, item) => sum + item.total, 0) - appliedVoucherAmount - (custPhone ? Math.round(cart.reduce((sum, item) => sum + item.total, 0) * getCustomerTier(customers[custPhone]?.totalSpent || 0).discountRate) : 0))) : 0));
    const logData = { id: Date.now(), shift, type: "BÁN", paymentMethod: method, qty: cart.reduce((s,i)=>s+i.qty, 0), total: finalTotalToPay, profit: 0, time: new Date().toLocaleString('vi-VN') };
    addTransactionAndSync(logData); setLastOrder(logData); resetCheckout(); setIsCheckoutOpen(false); toast.success("Thanh toán thành công!");
  };
  
  const sendReceiptEmail = () => toast.success("Hóa đơn đã được gửi!");
  const closeCheckout = () => { resetCheckout(); setIsCheckoutOpen(false); };
  ...
  const addSupplier = (e: React.FormEvent) => {
    e.preventDefault();
    setSuppliers(prev => [{ id: Date.now(), name: supName, phone: supPhone, address: supAddress, taxCode: supTaxCode, bankAccount: supBankAccount, items: supItem }, ...prev]);
    setSupName(""); setSupPhone(""); setSupAddress(""); setSupTaxCode(""); setSupBankAccount(""); setSupItem(""); toast.success("Thêm nhà cung cấp thành công!");
  };
  ...
  const printCustomerCard = () => setPrintMode('customer');
  const sendCardEmail = () => toast.success("Đã gửi thẻ hội viên qua Email!");
  const shareToZalo = () => toast.success("Đã chia sẻ thẻ qua Zalo!");
