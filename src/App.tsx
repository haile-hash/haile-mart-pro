...
export default function App() {
  ...
  const handleNextToQR = () => {
    if (custPhone && !customers[custPhone] && !custName) return alert("Vui lòng nhập Tên khách hàng mới!");
    setCheckoutStep(2);
  };
  ...
  const styles = `
    ...
    .glass-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
    body { background-color: #0f172a; margin: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
     ...
     return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", position: "relative", overflow: "hidden" }}>
        ...
        <div className="glass-card" style={{ padding: "40px", width: "100%", maxWidth: "400px", textAlign: "center" }}>
          <h1 style={{ color: "#1e293b", fontSize: "28px", marginBottom: "20px" }}>🏪 HẢI LÊ MART</h1>
          <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
            <input placeholder="Tài khoản" value={authUsername} onChange={e => setAuthUsername(e.target.value)} style={{ padding: "14px", borderRadius: "10px", outline: "none", border: "1px solid #cbd5e1" }} />
            <input type="password" placeholder="Mật khẩu" value={authPassword} onChange={e => setAuthPassword(e.target.value)} style={{ padding: "14px", borderRadius: "10px", outline: "none", border: "1px solid #cbd5e1" }} />
            <button type="submit" style={{ padding: "14px", backgroundColor: "#3b82f6", color: "#fff", border: "none", borderRadius: "10px", fontWeight: "bold", cursor: "pointer" }}>ĐĂNG NHẬP</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      {lastOrder && (
        <div className="print-only">
          <div style={{ textAlign: "center", marginBottom: "10px" }}>
            <h2 style={{ margin: "0 0 5px 0", fontSize: "22px" }}>HẢI LÊ MART</h2>
            <div style={{ fontSize: "12px" }}>Tòa Nhà ATS, 252 Hoàng Quốc Việt, HN<br/>Hotline: 0902613899</div>
          </div>
              ...
              {lastOrder.cart.map((item: any, idx: number) => {
                ...
                return (
                  <React.Fragment key={idx}>
                    ...
                    <tr><td style={{ color: "#555" }}>{isPromo ? <span><del style={{fontSize:"10px"}}>{item.product.sale_price.toLocaleString()}</del> {(item.product.promo_price).toLocaleString()}</span> : item.product.sale_price.toLocaleString()}</td><td style={{ textAlign: "center" }}>{item.qty}</td><td style={{ textAlign: "right" }}>{item.total.toLocaleString()}đ</td></tr>
                  </React.Fragment>
                )
              })}
            </tbody>
      ...
      <div className="no-print" style={{ padding: "20px", position: "relative", minHeight: "100vh" }}>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.8)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999 }}>
              ...
              <div className="glass-card" style={{ padding: "30px", width: "400px" }}>
                ...
                {custPhone && (
                  <div style={{ marginTop: "15px", padding: "15px", backgroundColor: "#f8fafc", borderRadius: "10px" }}>
                    ...
                    ) : <input type="text" placeholder="Tên khách mới" value={custName} onChange={e => setCustName(e.target.value)} style={{ width: "100%", padding: "10px", borderRadius: "8px", outline: "none" }} />}
                  </div>
                )}
                <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
                  <button onClick={() => setIsCheckoutOpen(false)} style={{ flex: 1, padding: "12px", borderRadius: "10px", border: "none", cursor: "pointer", background: "#e2e8f0", fontWeight: "bold" }}>Hủy</button>
            ...
            {checkoutStep === 2 && (
              <div className="glass-card" style={{ padding: "30px", width: "400px", textAlign: "center" }}>
                ...
                <div style={{ color: "#ef4444", fontSize: "32px", fontWeight: "900", marginBottom: "15px" }}>{Math.max(0, cartTotalAmount - (useWallet ? customers[custPhone]?.wallet || 0 : 0)).toLocaleString()}đ</div>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  <button onClick={() => setCheckoutStep(1)} style={{ flex: 1, padding: "12px", borderRadius: "10px", border: "none", cursor: "pointer", background: "#e2e8f0", fontWeight: "bold" }}>Quay lại</button>
            ...
            {checkoutStep === 3 && lastOrder && (
              <div className="glass-card" style={{ padding: "30px", width: "400px", textAlign: "center" }}>
                <div style={{ fontSize: "50px" }}>🎉</div><h3 style={{ color: "#16a34a", margin: "10px 0" }}>Thành công!</h3>
                <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
        ...
        <div style={{ maxWidth: "1350px", margin: "0 auto" }}>
          ...
          {/* THỐNG KÊ RÚT GỌN CHÚT CHO GỌN GÀNG */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "15px", marginBottom: "20px" }}>
            {[ { label: "GIÁ TRỊ KHO (VỐN)", val: totalValue, c: "#8b5cf6" }, { label: "DOANH THU HÔM NAY", val: revenue, c: "#3b82f6" }, { label: "LỢI NHUẬN ƯỚC TÍNH", val: profit, c: "#10b981" } ].map((s, i) => (
              <div key={i} className="glass-card" style={{ padding: "15px", borderLeft: `6px solid ${s.c}` }}>
                <div style={{ color: "#64748b", fontSize: "11px", fontWeight: "bold", marginBottom: "5px" }}>{s.label}</div>
                <div style={{ fontSize: "20px", fontWeight: "bold", color: "#1e293b" }}>{s.val.toLocaleString()}đ</div>
              </div>
            ))}
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "2.8fr 1fr", gap: "20px" }}>
            {/* CỘT TRÁI - QUẢN LÝ SẢN PHẨM */}
            <div className="glass-card" style={{ padding: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px", alignItems: "center" }}>
                <h2 style={{ color: "#1e3a8a", margin: 0, fontSize: "22px" }}>🏪 HẢI LÊ MART</h2>
                <button onClick={handleLogout} style={{ padding: "6px 12px", backgroundColor: "#fee2e2", color: "#ef4444", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}>Khóa 🔒</button>
              </div>

              {/* Ô TÍT MÃ */}
              <input placeholder="🔍 TÍT MÃ VẠCH ĐỂ BÁN HÀNG..." value={barcodeInput} onChange={e => setBarcodeInput(e.target.value)} onKeyDown={handleBarcodeSubmit} style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "2px solid #3b82f6", marginBottom: "15px", fontSize: "16px", fontWeight: "bold", outline: "none", background: "#eff6ff", boxSizing: "border-box" }} />

              {/* FORM NHẬP KHO GỌN GÀNG (SỬ DỤNG CSS GRID CHUẨN MỰC) */}
              <form onSubmit={handleAddProduct} style={{ backgroundColor: "#f8fafc", padding: "12px 15px", borderRadius: "10px", border: "1px solid #e2e8f0", marginBottom: "20px" }}>
                <div style={{ fontSize: "12px", fontWeight: "bold", color: "#475569", marginBottom: "10px" }}>📦 NHẬP KHO & CÀI ĐẶT KHUYẾN MÃI</div>
                
                {/* HÀNG 1 */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                  <input placeholder="Mã hàng..." value={newCode} onChange={handleCodeChange} style={{ padding: "8px", borderRadius: "6px", border: "1px solid #cbd5e1", outline: "none" }} />
                  <input placeholder="Tên sản phẩm..." value={newName} onChange={e => setNewName(e.target.value)} style={{ padding: "8px", borderRadius: "6px", border: "1px solid #cbd5e1", outline: "none" }} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.5fr 0.8fr 80px", gap: "10px" }}>
                  <input type="number" placeholder="Giá KM (nếu có)" value={newPromoPrice} onChange={e => setNewPromoPrice(e.target.value)} style={{ padding: "8px", borderRadius: "6px", border: "1px solid #f59e0b", background: "#fffbeb", outline: "none" }} title="Giá Khuyến Mãi" />
                  <input type="date" title="Hạn sử dụng" value={newExpiry} onChange={e => setNewExpiry(e.target.value)} style={{ padding: "8px", borderRadius: "6px", border: "1px solid #cbd5e1", color: "#475569", outline: "none" }} />
                  <input type="text" placeholder="Quà tặng (VD: 1 cốc)" value={newGiftInfo} onChange={e => setNewGiftInfo(e.target.value)} style={{ padding: "8px", borderRadius: "6px", border: "1px solid #10b981", background: "#f0fdf4", outline: "none" }} />
                  ...
                  <button type="submit" disabled={loading} style={{ padding: "8px", backgroundColor: "#1e3a8a", color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer" }}>LƯU</button>
                </div>
              </form>

              {/* THANH TÌM KIẾM SẢN PHẨM KHÔI PHỤC LẠI */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                <div style={{ fontSize: "14px", fontWeight: "bold", color: "#1e3a8a" }}>📋 DANH SÁCH SẢN PHẨM</div>
                <input placeholder="🔍 Tìm theo tên hoặc mã..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ padding: "8px 15px", borderRadius: "20px", border: "1px solid #cbd5e1", outline: "none", width: "250px", fontSize: "13px" }} />
              </div>
                    ...
                    return (
                      <tr key={p.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                        <td style={{ padding: "10px 8px" }}>
                          ...
                          {p.gift_info ? <div style={{ fontSize: "11px", color: "#10b981", fontWeight: "bold", marginTop: "3px", cursor: "pointer" }} onClick={()=>handleEdit(p.id, 'gift_info', p.gift_info, true)}>🎁 Tặng: {p.gift_info}</div> : <div style={{ fontSize: "10px", color: "#cbd5e1", cursor: "pointer" }} onClick={()=>handleEdit(p.id, 'gift_info', '', true)}>+ Thêm quà tặng</div>}
                        </td>
                        <td style={{ textAlign: "center", fontSize: "14px" }}><b>{p.stock}</b></td>
                        <td style={{ textAlign: "center", color: "#64748b", cursor: "pointer", fontSize: "13px" }} onClick={()=>handleEdit(p.id, 'import_price', p.import_price)}>{p.import_price?.toLocaleString()}đ</td>
                        <td style={{ textAlign: "center" }}>
                          <div style={{ color: isPromo ? "#94a3b8" : "#059669", textDecoration: isPromo ? "line-through" : "none", fontWeight: isPromo ? "normal" : "bold", fontSize: isPromo ? "11px" : "14px", cursor: "pointer" }} onClick={()=>handleEdit(p.id, 'sale_price', p.sale_price)}>
                            {p.sale_price.toLocaleString()}đ
                          </div>
                          {isPromo ? <div style={{ color: "#ef4444", fontWeight: "900", fontSize: "14px", cursor: "pointer" }} onClick={()=>handleEdit(p.id, 'promo_price', p.promo_price)}>🔥 {p.promo_price.toLocaleString()}đ</div> : <div style={{ fontSize: "10px", color: "#cbd5e1", cursor: "pointer" }} onClick={()=>handleEdit(p.id, 'promo_price', 0)}>+ Setup Giá KM</div>}
                        </td>
                        ...
                        <td style={{ textAlign: "right" }}>
                          <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: "8px" }}>
                            <button onClick={() => addToCart(p)} style={{ padding: "6px 10px", backgroundColor: "#f59e0b", color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer", fontSize: "11px" }}>+ VÀO GIỎ</button>
                            <button onClick={() => handleDelete(p.id, p.name)} style={{ background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: "14px", padding: 0 }} title="Xóa">🗑️</button>
                          </div>
                        </td>
                      </tr>
                    )
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div className="glass-card" style={{ padding: "20px" }}>
                <h3 style={{ margin: "0 0 15px 0", color: "#d97706", fontSize: "16px" }}>🛒 GIỎ HÀNG ({cart.length})</h3>
                <div style={{ maxHeight: "250px", overflowY: "auto", marginBottom: "15px" }}>
                  {cart.length === 0 && <div style={{textAlign: "center", color: "#94a3b8", fontSize: "12px"}}>Giỏ hàng trống</div>}
                  {cart.map((item, idx) => (
                    <div key={idx} style={{ padding: "8px 0", borderBottom: "1px dashed #e2e8f0", fontSize: "13px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <button onClick={()=>removeFromCart(item.product.id)} style={{border:"none",background:"none",color:"#ef4444", cursor: "pointer", fontSize: "16px"}}>×</button>
                      </div>
                      {item.product.gift_info && <div style={{ color: "#10b981", fontSize: "11px", fontStyle: "italic", marginTop: "2px" }}>+ 🎁 Tặng {item.qty} {item.product.gift_info.replace('Tặng ', '')}</div>}
                      <div style={{ color: "#ef4444", fontWeight: "bold", textAlign: "right", marginTop: "2px" }}>{item.total.toLocaleString()}đ</div>
                    </div>
                  ))}
                </div>
                {cart.length > 0 && (
                  <button onClick={() => { setIsCheckoutOpen(true); setCheckoutStep(1); }} style={{ width: "100%", padding: "12px", backgroundColor: "#10b981", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "bold", fontSize: "15px", cursor: "pointer" }}>
              ...
              <div className="glass-card" style={{ padding: "20px", flex: 1, display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: "5px" }}>
                    <button onClick={exportToCSV} style={{ fontSize: "10px", padding: "5px 8px", background: "#10b981", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>EXCEL</button>
                    <button onClick={handleSendEmailReport} style={{ fontSize: "10px", padding: "5px 8px", background: "#ef4444", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>✉️ CHỐT CA</button>
                  </div>
                </div>
                <div style={{ flex: 1, overflowY: "auto", paddingRight: "5px" }}>
                  {Object.keys(history.reduce((g:any, l:any)=>{const d=new Date(Math.floor(l.id)).toLocaleDateString('vi-VN');if(!g[d])g[d]=[];g[d].push({...l,t:new Date(Math.floor(l.id)).toLocaleTimeString('vi-VN')});return g;},{})).length === 0 && <div style={{textAlign: "center", color: "#94a3b8", fontSize: "12px", marginTop: "20px"}}>Chưa có dữ liệu</div>}
                  {Object.keys(history.reduce((g:any, l:any)=>{const d=new Date(Math.floor(l.id)).toLocaleDateString('vi-VN');if(!g[d])g[d]=[];g[d].push({...l,t:new Date(Math.floor(l.id)).toLocaleTimeString('vi-VN')});return g;},{})).map((dateStr) => {
                    ...
                    return (
                      <div key={dateStr} style={{ marginBottom: "10px", backgroundColor: "#f8fafc", borderRadius: "8px", overflow: "hidden", border: "1px solid #e2e8f0" }}>
                        <div onClick={() => toggleDateGroup(dateStr)} style={{ backgroundColor: "#f1f5f9", padding: "8px 12px", fontSize: "12px", fontWeight: "bold", color: "#334155", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <span>📅 {dateStr}</span>
                        ...
                        {isExpanded && (
                          <div style={{ padding: "0 12px" }}>
                            {group.map((log: any) => (
                              <div key={log.id} style={{ padding: "8px 0", borderBottom: "1px dashed #e2e8f0", fontSize: "11px", display: "flex", flexDirection: "column", gap: "4px" }}>
                                <div style={{ display: "flex", justifyContent: "space-between" }}>
