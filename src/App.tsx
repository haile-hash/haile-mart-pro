...
export default function App() {
  ...
  // 4. STATE KIỂM KHO & THÊM MỚI SẢN PHẨM
  ...
  // 5. STATE TẠP VỤ (CHI PHÍ, NCC, MKT)
  ...
  // 9. DỮ LIỆU LOCAL (SẼ ĐƯỢC SYNC LÊN CLOUD)
  ...
  // GỌI CUSTOM HOOK ĐỒNG BỘ ĐÁM MÂY TẠI ĐÂY
  ...
  const sendReceiptEmail = async () => {
    ...
    if (!email) return; 
    email = email.trim(); const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) return alert("❌ Lỗi: Địa chỉ Email không hợp lệ! Vui lòng kiểm tra lại (vd: ten@gmail.com).");
    ...
    lastOrder.cart.forEach((item: any) => { const priceToUse = item.priceIncludingVat !== undefined ? item.priceIncludingVat : Math.round(getActualPrice(item.product) * (1 + VAT_RATE)); itemsTable += `- ${cleanName(item.product.name)} x ${item.qty} = ${(priceToUse * item.qty).toLocaleString()}đ\n` }); 
    ...
    try { await (window as any).emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, emailData); alert("🚀 Đã gửi HĐ cho khách!"); logAudit("GỬI HĐ MAIL", `Gửi tới ${email}`); } catch (error: any) { console.error(error); alert(`❌ Lỗi EmailJS`); } 
    setLoading(false)
  };
  
  const sendCardEmail = async (phone: string) => {
    const cust = customers[phone]; let email = cust.email || window.prompt(`Nhập Email của ${cust.name}:`, "");
    if (!email) return; email = email.trim(); const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) return alert("❌ Lỗi: Địa chỉ Email không hợp lệ!");
    ...
    try { await (window as any).emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_VIP_ID, emailData); alert("🚀 Đã gửi Thẻ VIP!"); logAudit("GỬI THẺ VIP", `Gửi tới ${email}`); } catch (error: any) { console.error(error); alert(`❌ Lỗi EmailJS`); } 
    setLoading(false)
  };
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    const finalGiftInfo = newGiftInfo.trim() !== "" ? `${newGiftCondition};;;${newGiftInfo}` : null; const baseCode = newCode.trim(); const formattedCat = formatCategoryStr(newCategory);
    const allVariants = products.filter(p => p.product_code === baseCode || String(p.product_code).startsWith(`${baseCode}-`)); const exist = allVariants.find(p => p.product_code === baseCode);
    let syncMsg = "";
    ...
    const processData = async (lines: any[]) => {
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const pCode = String(cols[0] || "").trim(); const pName = String(cols[1] || "").trim(); const pCategory = formatCategoryStr(String(cols[2] || "")); 
          const pImpPrice = parseInt(String(cols[3] || "0").replace(/[,.]/g, '')) || 0; const pSalePrice = parseInt(String(cols[4] || "0").replace(/[,.]/g, '')) || 0; 
          const pPromoPrice = parseInt(String(cols[5] || "0").replace(/[,.]/g, '')) || 0; const pGift = cols[6] ? String(cols[6]).trim() : null; 
          const pStock = parseInt(String(cols[7] || "0").replace(/[,.]/g, '')) || 0; const pExpiry = cols[8] ? String(cols[8]).trim() : null; 
        ...
        logAudit("NHẬP FILE", `Nhập ${successCount} mã`); alert(`✅ Nhập thành công ${successCount} sản phẩm từ file!`); fetchProducts();
      } catch (err) { console.error(err); alert("Lỗi xử lý dữ liệu file, vui lòng kiểm tra lại định dạng."); } 
      setLoading(false);
    }; 
  ...
  const handleImportInventoryCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const processData = (lines: any[]) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        const cols = lines[i]; if (!cols || !Array.isArray(cols) || cols.join('').trim() === '') continue;
        const pCode = String(cols[0] || "").trim(); const actualVal = parseInt(String(cols[3] || "0").replace(/[,.]/g, ''));
      ...
      setActualStockInput(updatedStock); alert(`✅ Đã nạp số liệu cho ${count} sản phẩm có thay đổi từ file!`);
    };
  ...
  const handleSendEmailReport = async () => {
    ...
    logs.forEach(l => { if (l.type === 'BÁN') sold += l.qty; if (l.type === 'BÁN' || l.type === 'THU NỢ' || l.type === 'TRẢ HÀNG') { if (l.paymentMethod === 'CHUYỂN KHOẢN') transfer += l.total; else if (l.paymentMethod === 'TIỀN MẶT' || l.paymentMethod === 'KẾT HỢP') { if(l.paymentMethod === 'KẾT HỢP' && l.split_cash) { cash += l.split_cash; transfer += (l.total - l.split_cash); } else { cash += l.total } } } prof += (l.profit || 0) });
    let adminEmail = window.prompt("Nhập Email Quản lý để nhận báo cáo:", ""); if(!adminEmail) return;
    adminEmail = adminEmail.trim(); const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; if (!emailRegex.test(adminEmail)) return alert("❌ Lỗi: Địa chỉ Email không hợp lệ!");
    setLoading(true);
  ...
  const sendInventoryAlertEmail = async () => {
    let adminEmail = window.prompt("Nhập Email Quản lý để nhận cảnh báo:", ""); if(!adminEmail) return;
    adminEmail = adminEmail.trim(); const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; if (!emailRegex.test(adminEmail)) return alert("❌ Lỗi: Địa chỉ Email không hợp lệ!");
    setLoading(true); const lowStock = products.filter(p => p.stock > 0 && p.stock < 10); const today = new Date().getTime(); const expiring = products.filter(p => p.expiry_date && (new Date(p.expiry_date).getTime() - today) / 86400000 <= 15);
  ...
  // ==========================================
  // HÀM RENDER COMPONENT CON (GIAO DIỆN)
  // ==========================================
        ...
        <div className="print-only">
          <div className="print-receipt-container">
            ...
            <div style={{ fontSize: "11px", marginBottom: "8px", lineHeight: "1.5" }}>
              {lastOrder.custPhone ? (<><div><b>Khách hàng:</b> {lastOrder.custName || 'Khách VIP'}</div><div><b>SĐT:</b> {lastOrder.custPhone}</div>{customers[lastOrder.custPhone]?.email && <div><b>Email:</b> {customers[lastOrder.custPhone].email}</div>}</>) : (<div><b>Khách hàng:</b> Khách lẻ</div>)}
            </div>
            <div style={{ borderBottom: "1px dashed #000", marginBottom: "8px" }}></div>
            ...
            <table style={{ width: "100%", fontSize: "16px", fontWeight: 900, borderCollapse: "collapse" }}>
              <tbody><tr><td>{lastOrder.debtAmount > 0 ? "NỢ:" : "TỔNG:"}</td><td style={{ textAlign: "right" }}>{Math.round(lastOrder.debtAmount > 0 ? lastOrder.debtAmount : lastOrder.finalTotal).toLocaleString()}đ</td></tr></tbody>
            </table>
            ...
            {lastOrder.paymentMethod === 'TIỀN MẶT' && lastOrder.customerGiven > lastOrder.finalTotal && (
              <table style={{ width: "100%", fontSize: "12px", marginTop: "4px", borderCollapse: "collapse" }}>
                <tbody><tr><td>Khách đưa:</td><td style={{ textAlign: "right" }}>{Math.round(lastOrder.customerGiven).toLocaleString()}đ</td></tr><tr><td><b>Trả lại:</b></td><td style={{ textAlign: "right" }}><b>{Math.round(lastOrder.customerGiven - lastOrder.finalTotal).toLocaleString()}đ</b></td></tr></tbody>
              </table>
            )}
            ...
            <div style={{ display: "flex", gap: "15px", flex: 1, overflow: "hidden" }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#f0fdf4", padding: "12px", borderRadius: "8px", border: "1px solid #bbf7d0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px dashed #86efac", paddingBottom: "8px", marginBottom: "10px" }}>
                  ...
                  <span style={{ fontWeight: "900", color: "#15803d" }}>{currentShiftCashFlow.thu.reduce((acc: any, i: any) => acc + i.amount, 0).toLocaleString()}đ</span>
                </div>
                <div style={{ overflowY: "auto", flex: 1, paddingRight: "5px" }}>
                  ...
                  {currentShiftCashFlow.thu.map((item: any, idx: number) => (
                    <div key={idx} style={{ padding: "8px 0", borderBottom: "1px dashed #bbf7d0", fontSize: "12px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div style={{ display: "flex", flexDirection: "column", maxWidth: "70%" }}>
                        <b style={{ color: "#14532d", wordBreak: "break-word" }}>{item.note}</b><span style={{ fontSize: "10px", color: "#15803d", marginTop: "2px" }}>{item.time}</span>
                      </div>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#fef2f2", padding: "12px", borderRadius: "8px", border: "1px solid #fecaca" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px dashed #fca5a5", paddingBottom: "8px", marginBottom: "10px" }}>
                  ...
                  <span style={{ fontWeight: "900", color: "#b91c1c" }}>{currentShiftCashFlow.chi.reduce((acc: any, i: any) => acc + i.amount, 0).toLocaleString()}đ</span>
                </div>
                <div style={{ overflowY: "auto", flex: 1, paddingRight: "5px" }}>
                  ...
                  {currentShiftCashFlow.chi.map((item: any, idx: number) => (
                    <div key={idx} style={{ padding: "8px 0", borderBottom: "1px dashed #fecaca", fontSize: "12px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div style={{ display: "flex", flexDirection: "column", maxWidth: "70%" }}>
                        <b style={{ color: "#7f1d1d", wordBreak: "break-word" }}>{item.note}</b><span style={{ fontSize: "10px", color: "#b91c1c", marginTop: "2px" }}>{item.time}</span>
                      </div>
          ...
          <div className="glass" style={{ padding: "25px", width: "600px", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <div className="chart-container-scroll">
                {chartData.map((d, i) => (
                  <div key={i} className="chart-bar-group"><div className="chart-val" style={{ visibility: d.showLabel && d.total > 0 ? 'visible' : 'hidden' }}>{(d.total / 1000).toFixed(0)}k</div><div className="chart-bar" style={{ height: d.height }}></div><div className="chart-label" style={{ visibility: d.showLabel ? 'visible' : 'hidden' }}>{d.label}</div></div>
                ))}
              </div>
            </div>
            <div style={{ display: "flex", gap: "20px" }}>
              <div style={{ flex: 1 }}>
                ...
                {topSelling.map((item, idx) => (
                  <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px dashed var(--border-glass)", fontSize: "11px" }}><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "120px" }}>{idx + 1}. {item[0]}</span><span style={{ fontWeight: "bold", color: "#10b981" }}>{item[1]}</span></div>
                ))}
              </div>
              <div style={{ flex: 1 }}>
                ...
                {products.filter(p => p.stock > 0 && p.stock < 10).slice(0, 5).map((p, idx) => (
                  <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px dashed var(--border-glass)", fontSize: "11px" }}><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "120px" }}>{cleanName(p.name)}</span><span style={{ fontWeight: "bold", color: "#ef4444" }}>Còn {p.stock}</span></div>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="glass" style={{ padding: "25px", width: "900px", maxWidth: "95vw", maxHeight: "85vh", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ overflowY: "auto", flex: 1, paddingRight: "5px" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead><tr style={{ borderBottom: "1px solid var(--border-glass)", color: "var(--text-muted)", textAlign: "left", position: "sticky", top: 0, background: "var(--bg-glass)", zIndex: 1 }}><th style={{ padding: "8px" }}>Sản phẩm</th><th style={{ padding: "8px", textAlign: "center" }}>Kho PM</th><th style={{ padding: "8px", textAlign: "center" }}>Thực tế</th><th style={{ padding: "8px", textAlign: "center" }}>Chênh lệch</th></tr></thead>
                <tbody>
                  {products.filter(p => {
                      const safeName = String(cleanName(p.name) || "").toLowerCase(); const safeCode = String(p.product_code || "").toLowerCase(); const term = String(inventorySearchTerm || "").toLowerCase();
                      const matchSearch = safeName.includes(term) || safeCode.includes(term); const actual = actualStockInput[p.id] !== undefined ? actualStockInput[p.id] : (Number(p.stock) || 0); const diff = actual - (Number(p.stock) || 0);
              ...
              {products.filter(p => { const safeName = String(cleanName(p.name) || "").toLowerCase(); const safeCode = String(p.product_code || "").toLowerCase(); const term = String(inventorySearchTerm || "").toLowerCase(); const matchSearch = safeName.includes(term) || safeCode.includes(term); const diff = (actualStockInput[p.id] !== undefined ? actualStockInput[p.id] : (Number(p.stock) || 0)) - (Number(p.stock) || 0); if (invFilter === 'DIFF') return matchSearch && diff !== 0; if (invFilter === 'MATCH') return matchSearch && diff === 0; return matchSearch; }).length === 0 && (<div style={{ textAlign: "center", color: "var(--text-muted)", padding: "20px" }}>Không tìm thấy sản phẩm khớp với bộ lọc.</div>)}
            </div>
        ...
        <div className="no-print" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.8)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999 }}>
          <div className="glass" style={{ padding: "25px", width: "650px", maxHeight: "80vh", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px solid var(--border-glass)", paddingBottom: "10px", marginBottom: "10px" }}>
              <div style={{ display: "flex", gap: "10px", alignItems: "center" }}><h2 style={{ margin: 0, color: "#334155" }}>🕵️ NHẬT KÝ HỆ THỐNG</h2><button onClick={exportAuditToCSV} style={{ fontSize: "10px", padding: "4px 8px", background: "#10b981", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>📥 XUẤT FILE</button></div>
            ...
            <div style={{ overflowY: "auto", flex: 1, fontSize: "12px", border: "1px solid var(--border-glass)", borderRadius: "8px", padding: "5px", background: "var(--bg-input)" }}>
              ...
              {auditLogs.map((log, idx) => (
                <div key={idx} onClick={() => setSelectedAuditLog(log)} style={{ padding: "10px", borderBottom: "1px dashed var(--border-glass)", cursor: "pointer", transition: "0.2s" }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(59, 130, 246, 0.1)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}><span style={{ fontWeight: "bold", color: "#b91c1c" }}>[{log.action}]</span><span style={{ color: "var(--text-muted)", fontSize: "11px" }}>{log.time}</span></div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", marginRight: "10px" }}>{log.detail}</span><span style={{ fontWeight: "bold", color: "#3b82f6" }}>{log.user_name} ({log.shift})</span></div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="no-print" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 10000 }} onClick={() => setSelectedAuditLog(null)}>
           <div className="glass" style={{ padding: "20px", width: "450px", maxWidth: "90%", background: "var(--bg-glass)" }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ fontSize: "13px", lineHeight: "1.6" }}>
                 <p style={{ margin: "5px 0" }}><b>Hành động:</b> {selectedAuditLog.action}</p><p style={{ margin: "5px 0" }}><b>Người thực hiện:</b> {selectedAuditLog.user_name} - {selectedAuditLog.shift}</p><p style={{ margin: "5px 0" }}><b>Thời gian:</b> {selectedAuditLog.time}</p><p style={{ margin: "5px 0" }}><b>Tóm tắt:</b> <span style={{ color: "#3b82f6" }}>{selectedAuditLog.detail}</span></p>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              ...
              {heldOrders.map((order, idx) => (
                <div key={order.id} style={{ padding: "10px", borderBottom: "1px dashed var(--border-glass)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg-input)", borderRadius: "8px", marginBottom: "8px" }}>
                  <div><div style={{ fontWeight: "bold" }}>Đơn #{idx + 1}</div><div style={{ fontSize: "11px", color: "var(--text-muted)" }}>⏰ {order.time}</div><div style={{ fontSize: "11px", color: "#b91c1c", fontWeight: "bold" }}>Gồm {order.cart.reduce((s: any, i: any) => s + (Number(i.qty) || 0), 0)} SP</div></div>
  ...
  return (
    <div onClick={() => { setOpenFilter(null); setShowSuggestions(false); setShowMainMenu(false) }}>
      ...
      {/* Thêm dòng này để web nhận CSS nhé */}
      <style>{styles}</style> 
      <style>{`
      ...
      {/* 👉 THAY LINK NHẠC MP3 CỦA BẠN VÀO ĐÂY (Trong mục src="...") 👈 */}
      <audio ref={bgMusicRef} loop src="https://cdn.pixabay.com/audio/2023/10/24/audio_338a0f5a7d.mp3" />
      
      <button className="music-btn no-print" onClick={toggleMusic}>
        {isMusicPlaying ? '⏸️ Tắt nhạc' : '🎧 Bật nhạc Chill'}
      </button>

      <input type="text" id="search-barcode" style={{position:'absolute', opacity: 0, height: 0, width: 0}} />
      
      {/* 1. GIAO DIỆN IN ẤN (ẨN) */}
      {renderPrintArea()}

      {/* 2. CÁC CỬA SỔ POP-UP */}
      {renderModals()}

      {/* 3. NẾU CHƯA ĐĂNG NHẬP -> HIỆN FORM LOGIN */}
      {!isLoggedIn ? (
        ...
        /* 4. NẾU ĐÃ ĐĂNG NHẬP -> HIỆN APP CHÍNH */
        <div className="no-print" style={{ padding: "15px", position: "relative", minHeight: "100vh", overflowX: "auto" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                  ...
                  <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
                    ...
                    <div className="cash-box" onClick={(e) => { e.stopPropagation(); setCashFlowModalInfo('TIỀN MẶT'); }} style={{ textAlign: "center", whiteSpace: "nowrap" }}><div style={{ fontSize: "10px", color: "var(--text-muted)", fontWeight: "bold" }}>TIỀN MẶT 👆</div><div style={{ fontSize: "15px", fontWeight: "900", color: currentShiftStats.cash < 0 ? "#ef4444" : "#059669" }}>{currentShiftStats.cash.toLocaleString()}đ</div></div>
                    <div className="cash-box" onClick={(e) => { e.stopPropagation(); setCashFlowModalInfo('CHUYỂN KHOẢN'); }} style={{ textAlign: "center", whiteSpace: "nowrap" }}><div style={{ fontSize: "10px", color: "var(--text-muted)", fontWeight: "bold" }}>CHUYỂN KHOẢN 👆</div><div style={{ fontSize: "15px", fontWeight: "900", color: "#3b82f6" }}>{currentShiftStats.transfer.toLocaleString()}đ</div></div>
          ...
          /* Nút bật nhạc */
          .music-btn {
            position: fixed; bottom: 30px; right: 30px; background: rgba(255,255,255,0.6); backdrop-filter: blur(10px);
            border: 1px solid rgba(255,255,255,0.8); border-radius: 50px; padding: 12px 25px; cursor: pointer;
            box-shadow: 0 4px 15px rgba(0,0,0,0.08); display: flex; align-items: center; gap: 8px;
            font-weight: 900; font-size: 14px; color: #475569; transition: all 0.3s; z-index: 100;
          }
          .music-btn:hover { background: #fff; transform: scale(1.05) translateY(-5px); color: #ef4444; }
        `}</style>
  ...
  return (
    <div onClick={() => { setOpenFilter(null); setShowSuggestions(false); setShowMainMenu(false) }}>
      ...
      {/* 3. GIAO DIỆN CHÍNH (HEADER, LƯỚI SP, GIỎ HÀNG) */}
      <div className="no-print" style={{ padding: "15px", position: "relative", minHeight: "100vh", overflowX: "auto" }}>
        <div style={{ maxWidth: "1500px", margin: "0 auto", minWidth: "1000px" }}>
