...
export default function App() {
  ...
  // 🔥 SỬA LỖI KHÔNG LƯU NHẬT KÝ & KHÔNG LƯU NỢ KHÁCH HÀNG (SỬ DỤNG BATCH)
  const confirmCheckout = async (payMethod: 'TIỀN MẶT' | 'CHUYỂN KHOẢN' | 'GHI NỢ' | 'KẾT HỢP' | 'QUẸT THẺ' | 'ZALO PAY') => {
    ...
    try {
      ...
      // Update Customer Debt/Wallet safely
      if (custPhone) {
        ...
        if (navigator.onLine) { 
            await supabase.from("customers").update({
                wallet: updatedCust.wallet,
                debt: updatedCust.debt,
                totalSpent: updatedCust.totalSpent,
  ...
  // ===============================================
  // 🔥 KHU VỰC CHỨA CÁC MODAL ĐÃ ĐƯỢC CHUẨN HÓA CSS
  // ===============================================
  const renderModals = () => {
