...
export default function App() {
  ...
  const handleEdit = async (id, field, oldVal) => {
    const newVal = window.prompt(`Nhập giá trị mới cho ${field}:`, oldVal);
    if (newVal !== null) {
      await supabase
        .from("products")
        .update({ [field]: parseInt(newVal) })
        .eq("id", id);
    }
  };
    ...
    >
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        >
          ...
          >
            <div
              style={{ color: "#bfdbfe", fontSize: "13px", fontWeight: "bold" }}
            >
              DOANH THU PHIÊN NÀY
            </div>
            <div style={{ fontSize: "24px", fontWeight: "bold" }}>
        ...
        >
          ...
          >
            ...
            <input
              type="text"
              placeholder="🔍 Tìm kiếm tên sản phẩm hoặc mã hàng..."
              value={searchTerm}
                  ...
                  .map((p) => (
                    ...
                    >
                      ...
                      <td style={{ textAlign: "center" }}>
                        <button
                          onClick={() => handleEdit(p.id, "stock", p.stock)}
                          style={{
                            border: "none",
                            background: p.stock < 5 ? "#fee2e2" : "#f1f5f9",
                            color: p.stock < 5 ? "#ef4444" : "#475569",
                            padding: "4px 10px",
                            borderRadius: "5px",
                            fontWeight: "bold",
                            cursor: "pointer",
                      ...
                      >
                        <span
                          onClick={() =>
                            handleEdit(p.id, "sale_price", p.sale_price)
                          }
                  ...
                  >
                    ...
                    >
                      <span style={{ fontWeight: "bold", fontSize: "14px" }}>
                        {log.name}{" "}
                        <small style={{ color: "#64748b" }}>x{log.qty}</small>
                      </span>
