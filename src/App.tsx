...
export default function App(){
  ...
  // ================= TÍNH NĂNG TỰ ĐỘNG LƯU ĐÁM MÂY (OFFLINE-FIRST) =================
  const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [syncStatus, setSyncStatus] = useState<'synced' | 'syncing' | 'error'>('synced');
  const isInitialMount = useRef(true);

  // Theo dõi trạng thái kết nối mạng thực tế
  useEffect(() => {
  ...
  const syncToCloud = async (tableName: string, dataArray: any, isObject = false) => {
    ...
    try {
      ...
      let formattedData = [];
      if (isObject) {
        formattedData = Object.keys(dataArray).map(key => ({
          phone: key,
          ...dataArray[key]
        }));
      } else {
      ...
      const { error } = await supabase
        .from(tableName)
        .upsert(formattedData, { onConflict: tableName === 'customers' ? 'phone' : 'id' });
  ...
  // Quản lý lưu trữ song song LocalStorage và Đám mây
  useEffect(() => {
    ...
    // Debounce ngầm đẩy lên mây (tránh lag khi thao tác dồn dập)
