...
export default function App() {
  ...
  const handleRefund = async (logId: any) => {
    executeWithAdminCheck(() => {
      ...
      const p = products.find(x => x.id === log.product_id); if (p && navigator.onLine) supabase.from("products").update({ stock: p.stock + refundQty }).eq("id", p.id);
    ...
    <div onClick={() => { setOpenFilter(null); setShowSuggestions(false); setShowMainMenu(false) }}>
      ...
      {isLoggedIn && (
        <button 
          ...
          style={{ position: 'absolute', top: '90px', left: '120px', zIndex: 900, background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px 15px', fontWeight: 'bold', boxShadow: '0 4px 6px -1px rgba(14, 165, 233, 0.3)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s', fontSize: '13px' }}
