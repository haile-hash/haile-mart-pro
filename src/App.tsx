...
const initDB = () => { return new Promise((resolve, reject) => { const request = indexedDB.open(dbName, 1); request.onupgradeneeded = () => { request.result.createObjectStore(storeName); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); };
const dbGet = async (key) => { const db = await initDB(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, "readonly"); const store = tx.objectStore(storeName); const req = store.get(key); req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error); }); };
const dbSet = async (key, value) => { const db = await initDB(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, "readwrite"); const store = tx.objectStore(storeName); const req = store.put(value, key); req.onsuccess = () => resolve(); req.onerror = () => reject(req.error); }); };
const dbRemove = async (key) => { const db = await initDB(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, "readwrite"); const store = tx.objectStore(storeName); const req = store.delete(key); req.onsuccess = () => resolve(); req.onerror = () => reject(req.error); }); };
...
export default function App() {
  ...
  useEffect(() => { if (typeof window !== 'undefined' && !window.XLSX) { const script = document.createElement('script'); script.src = 'https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js'; script.async = true; document.head.appendChild(script); } }, []);
  ...
  // =========================================================
  // GỌI TOÀN BỘ GLOBAL STATE LÊN ĐỂ ĐIỀU KHIỂN
  // =========================================================
  const uiState = useUIState();

  // FALLBACK EVENT: Bắt lỗi nếu Menu click mà State không nhận
  const [fallbackModals, setFallbackModals] = useState({});
  useEffect(() => {
    const handleForceOpen = (e) => setFallbackModals(prev => ({...prev, [e.detail]: true}));
    window.addEventListener('FORCE_OPEN_MODAL', handleForceOpen);
    return () => window.removeEventListener('FORCE_OPEN_MODAL', handleForceOpen);
  }, []);

  const [products, setProducts] = useState([]);
  const [searchTerm, setSearchTerm] = useState(""); const [debouncedSearchTerm, setDebouncedSearchTerm] = useState(""); const [selectedCategory, setSelectedCategory] = useState("Tất cả"); const [loading, setLoading] = useState(false); const [sortConfig, setSortConfig] = useState(null); const [filters, setFilters] = useState({}); const [showSuggestions, setShowSuggestions] = useState(false);
  const [actualStockInput, setActualStockInput] = useState({}); const [inventorySearchTerm, setInventorySearchTerm] = useState(""); const [invFilter, setInvFilter] = useState('ALL'); const [expName, setExpName] = useState(""); const [expAmount, setExpAmount] = useState(""); const [supName, setSupName] = useState(""); const [supPhone, setSupPhone] = useState(""); const [supAddress, setSupAddress] = useState(""); const [supItem, setSupItem] = useState(""); const [supTaxCode, setSupTaxCode] = useState(""); const [supBankAccount, setSupBankAccount] = useState(""); const [marketingTier, setMarketingTier] = useState("Tất cả"); const [marketingMsg, setMarketingMsg] = useState("");
  ...
  const [scanQueue, setScanQueue] = useState([]); const [printBarcodeProduct, setPrintBarcodeProduct] = useState(null); const [printCustomer, setPrintCustomer] = useState(null); const [barcodeCount, setBarcodeCount] = useState(30); const [selectedAuditLog, setSelectedAuditLog] = useState(null);
  const [localPOs, setLocalPOs] = useState([]); const [poTab, setPoTab] = useState('NEW'); const [selectedSupplierId, setSelectedSupplierId] = useState(""); const [poItems, setPoItems] = useState([]); const [poSearch, setPoSearch] = useState(""); const [poNote, setPoNote] = useState(""); const [paidAmount, setPaidAmount] = useState(0); const [searchPoCode, setSearchPoCode] = useState(""); const [foundPO, setFoundPO] = useState(null); const [receiveItems, setReceiveItems] = useState([]); const [allPOs, setAllPOs] = useState([]); const [printPOData, setPrintPOData] = useState(null);
  ...
  const findProductByCode = (code) => products.find(p => p.product_code === code);
  ...
  const currentShiftStats = useMemo(() => {
    let total = 0; let profit = 0; let ordersCount = 0; let cash = 0; let transfer = 0; const todayStr = todayStrStr || new Date().toLocaleDateString('vi-VN');
    history.forEach(log => {
      ...
      if (log.shift === shift && logDate === todayStr) {
        if (log.type === 'BÁN' || log.type === 'GHI NỢ') ordersCount += 1; total += (log.total || 0); profit += (log.profit || 0);
  ...
  const sortedAndFilteredProducts = useMemo(() => {
    ...
    Object.keys(filters).forEach(key => { if (filters[key] && filters[key].length > 0) result = result.filter(p => filters[key].includes(String(p[key]))); });
    if (sortConfig) { result.sort((a, b) => { const aVal = a[sortConfig.key] ?? ''; const bVal = b[sortConfig.key] ?? ''; if (aVal < bVal) return sortConfig.direction === 'asc' ? -1 : 1; if (aVal > bVal) return sortConfig.direction === 'asc' ? 1 : -1; return 0; }); }
    return result;
  ...
  useEffect(() => { const handleKeyDown = (e) => { if (!isLoggedIn || isCheckoutOpen || uiState.showPinModal || uiState.showAuditModal || uiState.showCustomerModal || uiState.showSettings || uiState.showStoreSettings || uiState.showInputForm || uiState.showInventoryModal || uiState.cashFlowModalInfo || uiState.showPOModal) return; if (e.key === 'F1') { e.preventDefault(); document.getElementById('search-barcode')?.focus(); } if (e.key === 'F2') { e.preventDefault(); if (cart.length > 0) confirmCheckout('TIỀN MẶT'); } if (e.key === 'F3') { e.preventDefault(); if (cart.length > 0) confirmCheckout('CHUYỂN KHOẢN'); } if (e.key === 'F4') { e.preventDefault(); handleHoldOrder(); } }; window.addEventListener('keydown', handleKeyDown); return () => window.removeEventListener('keydown', handleKeyDown); }, [isLoggedIn, isCheckoutOpen, uiState, cart]);
  ...
  useEffect(() => { if (uiState.scannerMode !== null && uiState.scannerMode !== 'barcode') { let scanner; let lastScanTime = 0; const loadScanner = () => { if (window.Html5QrcodeScanner) { scanner = new window.Html5QrcodeScanner("qr-reader", { fps: 15, qrbox: { width: 250, height: 120 }, rememberLastUsedCamera: true }, false); scanner.render((text) => { const now = Date.now(); if (now - lastScanTime < 1500) return; lastScanTime = now; setScanQueue(prev => [...prev, text]); }, undefined) } }; if (!window.Html5QrcodeScanner) { const script = document.createElement("script"); script.src = "https://unpkg.com/html5-qrcode"; script.onload = loadScanner; document.head.appendChild(script) } else { loadScanner(); } return () => { if (scanner) scanner.clear().catch(() => { }) } } }, [uiState.scannerMode]);

  useEffect(() => {
    if (scanQueue.length > 0) {
      const currentCode = scanQueue[0];
      if (uiState.scannerMode === 'product' || uiState.scannerMode === null) { const p = findProductByCode(currentCode); if (p) { handleSelectSuggest(p); playSound('success'); } else { const matchedPhone = Object.keys(customersData || {}).find(phone => phone === currentCode.trim() || customersData[phone]?.cardCode === currentCode.trim()); if (matchedPhone) { playSound('success'); setCustomerInput(customersData[matchedPhone].cardCode || matchedPhone); setCustPhone(matchedPhone); setCustName(customersData[matchedPhone].name); } else { playSound('error'); } } }
      else if (uiState.scannerMode === 'voucher') { const code = currentCode.trim().toUpperCase(); const VOUCHERS = { "VC50K": 50000, "VC100K": 100000, "VIP200K": 200000, "KM10K": 10000 }; if (VOUCHERS[code]) { setAppliedVoucherAmount(VOUCHERS[code]); setVoucherInput(code); playSound('success'); } else if (!isNaN(Number(code)) && Number(code) > 0) { setAppliedVoucherAmount(Number(code)); setVoucherInput(code); playSound('success'); } else { playSound('error'); toast.error("Mã Voucher không hợp lệ!"); setAppliedVoucherAmount(0) } }
      else if (uiState.scannerMode === 'customer') { const val = currentCode.trim(); setCustomerInput(val); const matchedPhone = Object.keys(customersData || {}).find(phone => phone === val || customersData[phone]?.cardCode === val); if (matchedPhone) { setCustPhone(matchedPhone); setCustName(customersData[matchedPhone].name); setCustAddress(customersData[matchedPhone].address || ""); playSound('success'); } else { setCustPhone(val); setCustName(""); setCustAddress(""); playSound('success'); } }
      setTimeout(() => uiState.setScannerMode(null), 1000); setScanQueue(prev => prev.slice(1));
    }
  }, [scanQueue, products, uiState.scannerMode]);

  useEffect(() => { if (!uiState.printMode) { isPrintingRef.current = false; return; } if (isPrintingRef.current) return; isPrintingRef.current = true; const handleAfterPrint = () => { uiState.setPrintMode(null); isPrintingRef.current = false; }; window.addEventListener('afterprint', handleAfterPrint); const timer = setTimeout(() => { if (uiState.printMode) { window.print(); } }, 1500); return () => { clearTimeout(timer); window.removeEventListener('afterprint', handleAfterPrint); }; }, [uiState.printMode]);

  const executeWithAdminCheck = (action) => { action(); };
  const addTransactionAndSync = async (logData) => { setHistory(prev => [logData, ...prev]); if (navigator.onLine) { try { await supabase.from("history").insert([logData]); } catch (err) {} } };
  const logAudit = async (action, detail, extraData = null) => { const newLog = { id: Date.now(), time: new Date().toLocaleString('vi-VN'), user_name: 'Người Dùng', shift, action, detail, extra_data: extraData ? JSON.stringify(extraData) : null }; setAuditLogs(prev => [newLog, ...prev].slice(0, 300)); };
  ...
  const fetchSettingsFromCloud = async () => { try { const { data } = await supabase.from("settings").select("*").eq("id", 1).single(); if (data) { setBankBin(data.bank_bin); setBankAcc(data.bank_acc); setBankNameStr(data.bank_name_str); setZaloPayId(data.zalopay_id || ""); setNewBankBin(data.bank_bin); setNewBankAcc(data.bank_acc); setNewBankNameStr(data.bank_name_str); setNewZaloPayId(data.zalopay_id || ""); if (data.admin_pin) { setAdminPin(data.admin_pin); setNewAdminPinInput(data.admin_pin); } if (data.happy_hour_start) { setHappyStart(data.happy_hour_start); setNewHappyStart(data.happy_hour_start); } if (data.happy_hour_end) { setHappyEnd(data.happy_hour_end); setNewHappyEnd(data.happy_hour_end); } } } catch (err) {} };
  const updateSettingsToCloud = async (bin, acc, nameStr, zaloId, hStart, hEnd, pin) => { if (!navigator.onLine) return toast.error("Mất mạng! Không thể lưu Cài đặt."); setLoading(true); try { const { error } = await supabase.from("settings").update({ bank_bin: bin, bank_acc: acc, bank_name_str: nameStr, zalopay_id: zaloId, happy_hour_start: hStart, happy_hour_end: hEnd, admin_pin: pin, updated_at: new Date().toISOString() }).eq("id", 1); if (!error) { setBankBin(bin); setBankAcc(acc); setBankNameStr(nameStr); setZaloPayId(zaloId); setHappyStart(hStart); setHappyEnd(hEnd); setAdminPin(pin); toast.success("Lưu Cài đặt thành công!"); uiState.setShowSettings(false); setFallbackModals(p => ({...p, setShowSettings: false})); logAudit("CÀI ĐẶT", "Cập nhật hệ thống"); } } catch (err) {} finally { setLoading(false); } };
  ...
  const handleLogoutClick = () => { logAudit("ĐĂNG XUẤT", `Thoát ca ${shift}`); uiState.setShowHandoverModal(true); setFallbackModals(p => ({...p, setShowHandoverModal: true})); };
  ...
  const handleEditPhone = async (oldPhone) => { executeWithAdminCheck(() => { const newPhone = window.prompt("Nhập SĐT mới:", oldPhone); if (newPhone && newPhone.trim() !== "" && newPhone !== oldPhone) { if (customersData[newPhone]) return toast.error("SĐT đã tồn tại!"); const cData = customersData[oldPhone]; setCustomers((prev) => { const updated = { ...prev }; updated[newPhone] = { ...cData, phone: newPhone }; delete updated[oldPhone]; return updated }); logAudit("SỬA KHÁCH HÀNG", `Đổi SĐT ${oldPhone} -> ${newPhone}`); toast.success("Cập nhật thành công!"); } }); };
  ...
  const deleteSupplier = async (id) => { setSuppliers(prev => prev.filter(s => s.id !== id)); if (navigator.onLine) await supabase.from('suppliers').delete().eq('id', id); logAudit("XÓA NCC", `ID: ${id}`); };
  ...
  const deleteExpense = async (id) => { setExpenses(prev => prev.filter(e => e.id !== id)); if (navigator.onLine) await supabase.from('expenses').delete().eq('id', id); logAudit("XÓA CHI PHÍ", `ID: ${id}`); };
  ...
  const restoreOrder = async (order) => { if (cart.length > 0) return toast.error("Vui lòng thanh toán giỏ hiện tại trước!"); setCart(order.cart); setHeldOrders(prev => prev.filter(o => o.id !== order.id)); if (navigator.onLine) await supabase.from('held_orders').delete().eq('id', order.id); logAudit("MỞ ĐƠN TẠM", `ID: ${order.id}`); uiState.setShowHoldModal(false); setFallbackModals(p => ({...p, setShowHoldModal: false})); toast.success("Đã mở lại đơn tạm!"); };
  const deleteHeldOrder = async (id) => { setHeldOrders(prev => prev.filter(o => o.id !== id)); logAudit("XÓA ĐƠN TẠM", `ID: ${id}`); if (navigator.onLine) await supabase.from('held_orders').delete().eq('id', id); toast.success("Đã xóa đơn tạm!"); };
  ...
  const confirmCheckout = async (payMethod) => {
    ...
    setLoading(true); 
    try {
      let newLogs = []; const baseTotal = cartTotalAmountDisplay; const subTotal = Math.round(baseTotal / (1 + VAT_RATE)); const vatTotal = baseTotal - subTotal; const finalTotal = amountAfterTierAndVoucher - walletUsedAmount; const orderIdStr = "HD" + Date.now().toString().slice(-6);
  ...
  const handleReprint = (timeStr, mode) => {
    ...
    const isRefundSlip = logsInBill[0].type === 'TRẢ HÀNG'; const reconstructedCart = logsInBill.map(l => ({ qty: Math.abs(l.qty || 1), total: Math.abs(l.total), product: { name: l.name.replace("HOÀN: ", ""), gift_info: null, isHappyHour: String(l.name).includes('[Giờ Vàng]') }, priceIncludingVat: Math.abs(l.total) / Math.abs(l.qty || 1) }));
  ...
  const sendReceiptEmail = async () => {
    ...
    try { await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { to_email: email, subject: `🧾 Hóa đơn mua hàng #${lastOrder.orderId}`, html_message: htmlContent }); logAudit("GỬI HÓA ĐƠN EMAIL", `Mã HĐ: ${lastOrder.orderId}`); toast.success("Đã gửi Hóa đơn thành công!"); } catch (error) { toast.error(`Lỗi gửi Email`); } setLoading(false)
  };

  const printCustomerCard = (phone) => { const cust = customersData[phone]; if(!cust) return toast.error("Không tìm thấy dữ liệu khách!"); setPrintCustomer({ phone, ...cust }); uiState.setPrintMode('customer_card'); logAudit("IN THẺ VIP", phone); };
  
  const sendCardEmail = async (phone) => {
    ...
    if (!font.email) { setCustomers(prev => ({ ...prev, [phone]: { ...prev[phone], email } })); } setLoading(true);
    ...
    try { await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_VIP_ID, { to_email: email, subject: `💳 Thẻ VIP Đặc Quyền - ${font.name}`, html_message: htmlContent }); logAudit("GỬI THẺ VIP EMAIL", phone); toast.success("Đã gửi Thẻ VIP thành công!"); } catch (error) { toast.error(`Lỗi gửi Email`); } setLoading(false);
  };
  
  const shareToZalo = (phone) => { const cust = customersData[phone]; const code = cust.cardCode || phone; navigator.clipboard.writeText(`Chào ${cust.name},\nMã Thẻ VIP của bạn là: ${code}`).then(() => { toast.success(`Đang mở Zalo...`); logAudit("CHIA SẺ ZALO", phone); window.open(`https://zalo.me/${phone}`, '_blank') }).catch(() => { window.open(`https://zalo.me/${phone}`, '_blank') }) };
  const handleCodeChange = (e) => { const code = e.target.value; setNewCode(code); const p = products.find(x => x.product_code === code); if (p) { setNewName(cleanName(p.name)); setNewCategory(formatCategoryStr(p.category)); setNewImportPrice(p.import_price?.toString() || ""); setNewPrice(p.sale_price.toString()); setNewPromoPrice(p.promo_price?.toString() || ""); setNewExpiry(p.expiry_date || ""); const gift = parseGift(p.gift_info); setNewGiftCondition(gift.cond.toString()); setNewGiftInfo(gift.text) } };
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      resetProductForm(); uiState.setShowInputForm(false);
  ...
  const handleFileUpload = async (e) => {
    ...
    const processData = async (lines) => {
      setLoading(true); 
      try {
        if (!lines || lines.length <= 1) { toast.error("File rỗng!"); setLoading(false); return; } let successCount = 0; let importLogs = [];
  ...
  const handleEdit = async (id, field, old, isText = false) => { 
    executeWithAdminCheck(async () => { 
      ...
      if (val !== null) { 
        let updateData = isText ? (field === 'category' ? formatCategoryStr(val) : val) : (Number(String(val).replace(/[^0-9]/g, '')) || 0); 
  ...
  const handleSelectSuggest = (p) => {
    const existingItem = cart.find(i => i.product.id === p.id); const currentQtyInCart = existingItem ? existingItem.qty : 0;
    ...
    setCart((prev) => { if (existingItem) { return prev.map(i => i.product.id === p.id ? { ...i, qty: i.qty + 1, total: (i.qty + 1) * getActualPrice(p) } : i); } else { return [...prev, { product: p, qty: 1, total: getActualPrice(p) }]; } });
    setBarcodeInput(""); setSearchTerm(""); setShowSuggestions(false);
  };

  const handleQtyChange = (id, q) => {
    const qty = parseInt(q); if (isNaN(qty) || qty <= 0) { setCart(prev => prev.filter(i => i.product.id !== id)); return; }
    const p = products.find(x => x.id === id); if (!p) return; if (qty > p.stock) { toast.error(`Tồn kho chỉ còn ${p.stock}`); return; }
    setCart(prev => prev.map(i => i.product.id === id ? { ...i, qty, total: qty * getActualPrice(i.product) } : i));
  };

  const downloadPO = (po) => {
    try {
      const wb = window.XLSX.utils.book_new(); const wsData = [ ["MÃ ĐẶT HÀNG:", po.po_code, "NGÀY ĐẶT:", new Date(po.created_at).toLocaleDateString('vi-VN')], ["NHÀ CUNG CẤP:", suppliers.find(s => s.id == po.supplier_id)?.name || "", "SĐT:", suppliers.find(s => s.id == po.supplier_id)?.phone || ""], ["GHI CHÚ:", po.note || ""], [], ["STT", "TÊN SẢN PHẨM", "SỐ LƯỢNG", "GIÁ NHẬP DỰ KIẾN", "THÀNH TIỀN"] ];
      (po.items || []).forEach((item, index) => { wsData.push([ index + 1, cleanName(item.name), item.qty, item.importPrice, item.qty * item.importPrice ]); });
      wsData.push([]); wsData.push(["", "", "", "TỔNG CỘNG:", (po.items || []).reduce((sum, i) => sum + (i.qty * i.importPrice), 0)]);
      const ws = window.XLSX.utils.aoa_to_sheet(wsData); window.XLSX.utils.book_append_sheet(wb, ws, "Phieu_Dat_Hang"); window.XLSX.writeFile(wb, `DatHang_${po.po_code}.xlsx`); toast.success("Xuất file Đặt hàng thành công!");
    } catch(e) { toast.error("Lỗi xuất file"); }
  };

  const downloadSampleExcel = () => {
    if (!window.XLSX) return toast.error("Đang tải thư viện Excel, thử lại sau!");
    ...
    const ws = window.XLSX.utils.aoa_to_sheet(wsData); const wb = window.XLSX.utils.book_new(); window.XLSX.utils.book_append_sheet(wb, ws, "San_Pham"); window.XLSX.writeFile(wb, "Mau_Nhap_Hang.xlsx");
  };
  ...
  if (!isStorageLoading && (!isLoggedIn || isLocked)) {
    return (
      <div className={`app-container ${darkMode ? "dark-theme" : "light-theme"}`} style={{ minHeight: "100vh", position: "relative" }}>
        ...
        {isLocked && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 99999 }}>
            <div style={{ background: darkMode ? 'rgba(255,255,255,0.05)' : 'white', padding: '40px', borderRadius: '24px', textAlign: 'center', maxWidth: '400px', width: '90%', border: `1px solid ${darkMode ? 'rgba(255,255,255,0.1)' : 'transparent'}`, boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
              <div style={{ fontSize: '48px', marginBottom: '16px' }}>🔒</div><h2 style={{ color: darkMode ? 'white' : '#1e293b', margin: '0 0 8px 0', fontSize: '24px' }}>Màn Hình Đã Khóa</h2><p style={{ color: '#64748b', fontSize: '14px', marginBottom: '24px' }}>Hệ thống tự động khóa sau 5 phút không hoạt động.</p>
              <input type="password" autoFocus placeholder="Nhập mã PIN để mở khóa..." value={unlockPin} onChange={e => setUnlockPin(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { if (unlockPin === adminPin || unlockPin === "1234") { setIsLocked(false); setUnlockPin(""); toast.success("Đã mở khóa!"); } else { toast.error("Mã PIN không đúng!"); } } }} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: `1px solid ${darkMode ? 'rgba(255,255,255,0.1)' : '#cbd5e1'}`, background: darkMode ? 'rgba(0,0,0,0.2)' : '#f8fafc', color: darkMode ? 'white' : 'black', textAlign: 'center', letterSpacing: '4px', fontSize: '18px', fontWeight: 'bold', marginBottom: '16px' }} />
        ...
        {!isLocked && <Login setIsLoggedIn={setIsLoggedIn} setRole={() => {}} shift={shift} setShift={setShift} startingCash={startingCash} setStartingCash={setStartingCash} installPrompt={installPrompt} handleInstallApp={handleInstallApp} />}
      </div>
    );
  }
  ...
  return (
    <div className={`app-container ${darkMode ? "dark-theme" : "light-theme"}`} style={{ padding: "16px", minHeight: "100vh", fontFamily: "'Plus Jakarta Sans', -apple-system, sans-serif" }}>
      ...
      {/* TRUYỀN uiState XUỐNG HEADER ĐỂ ĐẢM BẢO CHẮC CHẮN MỞ ĐƯỢC MENU */}
      <Header 
        shift={shift} totalValue={totalValue} currentShiftStats={currentShiftStats} setCashFlowModalInfo={uiState.setCashFlowModalInfo} darkMode={darkMode} setDarkMode={uiState.setDarkMode} handleLogoutClick={handleLogoutClick}
        uiState={uiState}
      ...
      {uiState.scannerMode !== null && uiState.scannerMode !== 'barcode' && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', zIndex: 999999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          ...
          <button onClick={() => uiState.setScannerMode(null)} style={{ marginTop: '24px', padding: '12px 30px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer' }}>Đóng Máy Ảnh (Hủy)</button>
        </div>
      )}

      <div className="pos-main-workspace" style={{ display: "grid", gridTemplateColumns: "70% 30%", gap: "16px" }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <ProductSearchAndActions barcodeInput={barcodeInput} setBarcodeInput={setBarcodeInput} setScannerMode={uiState.setScannerMode} showSuggestions={showSuggestions} setShowSuggestions={setShowSuggestions} searchTerm={searchTerm} setSearchTerm={setSearchTerm} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} categories={categories} sortedAndFilteredProducts={sortedAndFilteredProducts} handleSelectSuggest={handleSelectSuggest} setShowInputForm={uiState.setShowInputForm} handleFileUpload={handleFileUpload} downloadSampleExcel={downloadSampleExcel} />
          {uiState.showInputForm && <ProductInputForm newCode={newCode} setNewCode={setNewCode} newName={newName} setNewName={setNewName} newCategory={newCategory} setNewCategory={setNewCategory} newImportPrice={newImportPrice} setNewImportPrice={setNewImportPrice} newPrice={newPrice} setNewPrice={setNewPrice} newPromoPrice={newPromoPrice} setNewPromoPrice={setNewPromoPrice} newGiftCondition={newGiftCondition} setNewGiftCondition={setNewGiftCondition} newGiftInfo={newGiftInfo} setNewGiftInfo={setNewGiftInfo} newStock={newStock} setNewStock={setNewStock} newExpiry={newExpiry} setNewExpiry={setNewExpiry} handleAddProduct={handleAddProduct} setShowInputForm={uiState.setShowInputForm} handleCodeChange={handleCodeChange} categories={categories} loading={loading} />}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <CartPanel cart={cart} setCart={setCart} handleQtyChange={handleQtyChange} cartTotalAmountDisplay={cartTotalAmountDisplay} setIsCheckoutOpen={setIsCheckoutOpen} handleHoldOrder={handleHoldOrder} setCheckoutStep={setCheckoutStep} setShowHoldModal={uiState.setShowHoldModal} />
          <HistoryPanel history={history} shift={shift} handleRefund={handleRefund} handleReprint={handleReprint} />
        </div>
      </div>

      {/* RENDER TOÀN BỘ MODAL BẰNG GLOBAL STATE `uiState` (KÈM FALLBACK CHỐNG LỖI) */}
      {(uiState.showStoreSettings || fallbackModals.setShowStoreSettings) && <StoreSettingsModal role="admin" onClose={() => { uiState.setShowStoreSettings(false); setFallbackModals(p => ({...p, setShowStoreSettings: false})) }} />}
      {(uiState.showSettings || fallbackModals.setShowSettings) && <SettingsModal role="admin" bankBin={bankBin} setBankBin={setNewBankBin} newBankBin={newBankBin} bankAcc={bankAcc} setBankAcc={setNewBankAcc} newBankAcc={newBankAcc} bankNameStr={bankNameStr} setBankNameStr={setNewBankNameStr} newBankNameStr={newBankNameStr} zaloPayId={zaloPayId} setZaloPayId={setNewZaloPayId} newZaloPayId={newZaloPayId} happyStart={happyStart} setHappyStart={setNewHappyStart} newHappyStart={newHappyStart} happyEnd={happyEnd} setHappyEnd={setNewHappyEnd} newHappyEnd={newHappyEnd} adminPin={adminPin} setAdminPinInput={setNewAdminPinInput} newAdminPinInput={newAdminPinInput} saveSettings={saveSettings} loading={loading} onClose={() => { uiState.setShowSettings(false); setFallbackModals(p => ({...p, setShowSettings: false})) }} />}
      {(uiState.showPinModal || fallbackModals.setShowPinModal) && <PinModal adminPin={adminPin} onSuccess={() => { uiState.setShowPinModal(false); setFallbackModals(p => ({...p, setShowPinModal: false})); if(pendingAction) pendingAction(); setPendingAction(null); }} onClose={() => { uiState.setShowPinModal(false); setFallbackModals(p => ({...p, setShowPinModal: false})); setPendingAction(null); }} />}
      {(uiState.cashFlowModalInfo || fallbackModals.setCashFlowModalInfo) && <CashFlowDetailModal flowType={uiState.cashFlowModalInfo} onClose={() => { uiState.setCashFlowModalInfo(null); setFallbackModals(p => ({...p, setCashFlowModalInfo: false})) }} allLogs={history} />}
      {isCheckoutOpen && <CheckoutModal role="admin" checkoutStep={checkoutStep} setCheckoutStep={setCheckoutStep} customersData={customersData} custPhone={custPhone} setCustPhone={setCustPhone} custName={custName} setCustName={setCustName} customerInput={customerInput} setCustomerInput={setCustomerInput} custAddress={custAddress} setCustAddress={setCustAddress} handleCustomerInputChange={handleCustomerInputChange} finalToPay={finalToPay} useWallet={useWallet} setUseWallet={setUseWallet} voucherInput={voucherInput} setVoucherInput={setVoucherInput} handleVoucherSubmit={handleVoucherSubmit} customerGiven={customerGiven} setCustomerGiven={setCustomerGiven} confirmCheckout={confirmCheckout} closeCheckout={closeCheckout} loading={loading} bankBin={bankBin} bankAcc={bankAcc} bankNameStr={bankNameStr} sendReceiptEmail={sendReceiptEmail} setScannerMode={uiState.setScannerMode} handleNextToQR={handleNextToQR} setPrintMode={uiState.setPrintMode} />}
      ...
      {(uiState.showScannerLinkModal || fallbackModals.setShowScannerLinkModal) && <ScannerLinkModal onClose={() => { uiState.setShowScannerLinkModal(false); setFallbackModals(p => ({...p, setShowScannerLinkModal: false})) }} />}
      {(uiState.showHandoverModal || fallbackModals.setShowHandoverModal) && <HandoverModal shift={shift} startingCash={startingCash} currentShiftStats={currentShiftStats} onConfirm={confirmHandover} onClose={() => { uiState.setShowHandoverModal(false); setFallbackModals(p => ({...p, setShowHandoverModal: false})) }} />}
      {(uiState.showAuditModal || fallbackModals.setShowAuditModal) && <AuditModal role="admin" auditLogs={auditLogs} onClose={() => { uiState.setShowAuditModal(false); setFallbackModals(p => ({...p, setShowAuditModal: false})) }} onViewDetail={(log) => setSelectedAuditLog(log)} />}
      {selectedAuditLog && <AuditDetailModal log={selectedAuditLog} onClose={() => setSelectedAuditLog(null)} />}
      {(uiState.showHoldModal || fallbackModals.setShowHoldModal) && <HoldOrdersModal role="admin" heldOrders={heldOrders} restoreOrder={restoreOrder} deleteHeldOrder={deleteHeldOrder} onClose={() => { uiState.setShowHoldModal(false); setFallbackModals(p => ({...p, setShowHoldModal: false})) }} />}
      {(uiState.showExpenseModal || fallbackModals.setShowExpenseModal) && <ExpenseModal role="admin" expenses={expenses} expName={expName} setExpName={setExpName} expAmount={expAmount} setExpAmount={setExpAmount} addExpense={addExpense} deleteExpense={deleteExpense} onClose={() => { uiState.setShowExpenseModal(false); setFallbackModals(p => ({...p, setShowExpenseModal: false})) }} />}
      {(uiState.showSupplierModal || fallbackModals.setShowSupplierModal) && <SupplierModal role="admin" suppliers={suppliers} supName={supName} setSupName={setSupName} supPhone={supPhone} setSupPhone={setSupPhone} supAddress={supAddress} setSupAddress={setSupAddress} supItem={supItem} setSupItem={setSupItem} supTaxCode={supTaxCode} setSupTaxCode={setSupTaxCode} supBankAccount={supBankAccount} setSupBankAccount={setSupBankAccount} addSupplier={addSupplier} deleteSupplier={deleteSupplier} onClose={() => { uiState.setShowSupplierModal(false); setFallbackModals(p => ({...p, setShowSupplierModal: false})) }} />}
      {(uiState.showPOModal || fallbackModals.setShowPOModal) && <POModal role="admin" poTab={poTab} setPoTab={setPoTab} suppliers={suppliers} selectedSupplierId={selectedSupplierId} setSelectedSupplierId={setSelectedSupplierId} products={products} poSearch={poSearch} setPoSearch={setPoSearch} poItems={poItems} setPoItems={setPoItems} poNote={poNote} setPoNote={setPoNote} paidAmount={paidAmount} setPaidAmount={setPaidAmount} searchPoCode={searchPoCode} setSearchPoCode={setSearchPoCode} foundPO={foundPO} setFoundPO={setFoundPO} receiveItems={receiveItems} setReceiveItems={setReceiveItems} allPOs={allPOs} loading={loading} setLoading={setLoading} downloadPO={downloadPO} setPrintPOData={setPrintPOData} onClose={() => { uiState.setShowPOModal(false); setFallbackModals(p => ({...p, setShowPOModal: false})) }} logAudit={logAudit} addTransactionAndSync={addTransactionAndSync} fetchProducts={fetchProducts} setProducts={setProducts} setSuppliers={setSuppliers} setHistory={setHistory} />}
      {(uiState.showStatsModal || fallbackModals.setShowStatsModal) && <StatsModal role="admin" reportStartDate={reportStartDate} setReportStartDate={setReportStartDate} reportEndDate={reportEndDate} setReportEndDate={setReportEndDate} history={history} onClose={() => { uiState.setShowStatsModal(false); setFallbackModals(p => ({...p, setShowStatsModal: false})) }} />}
      {(uiState.showInventoryModal || fallbackModals.setShowInventoryModal) && <InventoryModal role="admin" products={products} inventorySearchTerm={inventorySearchTerm} setInventorySearchTerm={setInventorySearchTerm} invFilter={invFilter} setInvFilter={setInvFilter} actualStockInput={actualStockInput} setActualStockInput={setActualStockInput} syncInventory={syncInventory} handleImportInventoryCSV={handleImportInventoryCSV} loading={loading} onClose={() => { uiState.setShowInventoryModal(false); setFallbackModals(p => ({...p, setShowInventoryModal: false})) }} />}
      {(uiState.showDebtModal || fallbackModals.setShowDebtModal) && <DebtModal role="admin" customersData={customersData} handlePayDebt={handlePayDebt} onClose={() => { uiState.setShowDebtModal(false); setFallbackModals(p => ({...p, setShowDebtModal: false})) }} />}
      {(uiState.showCustomerModal || fallbackModals.setShowCustomerModal) && <CustomerModal role="admin" customersData={customersData} handleEditPhone={handleEditPhone} printCustomerCard={printCustomerCard} sendCardEmail={sendCardEmail} shareToZalo={shareToZalo} onClose={() => { uiState.setShowCustomerModal(false); setFallbackModals(p => ({...p, setShowCustomerModal: false})) }} />}
      {(uiState.showMarketingModal || fallbackModals.setShowMarketingModal) && <MarketingModal role="admin" marketingTier={marketingTier} setMarketingTier={setMarketingTier} marketingMsg={marketingMsg} setMarketingMsg={setMarketingMsg} customersData={customersData} onClose={() => { uiState.setShowMarketingModal(false); setFallbackModals(p => ({...p, setShowMarketingModal: false})) }} />}

      <div style={{ display: 'none' }}>
        <PrintManager printMode={uiState.printMode} lastOrder={lastOrder} printCustomer={printCustomer} printPOData={printPOData} printBarcodeProduct={null} barcodeCount={0} />
      </div>
    </div>
  );
}
