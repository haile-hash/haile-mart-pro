...
export const useCheckoutState = () => {
  const [custAddress, setCustAddress] = useState(""); const [cart, setCart] = useState<any[]>([]);
  const [barcodeInput, setBarcodeInput] = useState(""); const [isCheckoutOpen, setIsCheckoutOpen] = useState(false);
  const [checkoutStep, setCheckoutStep] = useState(1); const [customerInput, setCustomerInput] = useState("");
  const [custPhone, setCustPhone] = useState(""); const [custName, setCustName] = useState("");
  const [useWallet, setUseWallet] = useState(false); const [voucherInput, setVoucherInput] = useState("");
  ...
  const resetCheckout = () => {
    setCustAddress(""); setCart([]); setIsCheckoutOpen(false); setCheckoutStep(1); setCustPhone("");
