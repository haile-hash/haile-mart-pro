...
export const MobileScanner = () => {
  ...
  useEffect(() => {
    ...
    const loadScanner = () => {
      if ((window as any).Html5QrcodeScanner) {
        scanner = new (window as any).Html5QrcodeScanner("qr-reader-mobile", { fps: 15, qrbox: { width: 250, height: 150 }, rememberLastUsedCamera: true, videoConstraints: { facingMode: "environment" } }, false);
        scanner.render(async (text: string) => {
          if (text && text !== lastScanRef.current) {
            lastScanRef.current = text; 
            const audio = new Audio("https://actions.google.com/sounds/v1/alarms/beep_short.ogg");
            audio.play().catch(()=>{}); toast.success(`Đã quét: ${text}`);
            await supabase.from('remote_scans').insert([{ code: text }]);
            setTimeout(() => { lastScanRef.current = ""; }, 400); 
          }
        }, undefined);
      }
    };
  ...
  return (
    <div style={{ padding: "20px", textAlign: "center", background: "#0f172a", minHeight: "100vh", color: "#fff", fontFamily: "sans-serif" }}>
      ...
      <p style={{ fontSize: "14px", color: "#94a3b8", marginBottom: "20px", padding: "0 20px" }}>Cứ lia camera qua mã vạch liên tục. Máy tính sẽ tự động cộng dồn số lượng! 🚀</p>
