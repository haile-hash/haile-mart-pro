        ...
        <div style={{ padding: "24px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "20px", background: "#fafafa" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
            ...
            <div style={{ background: "#ffffff", padding: "20px", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.02)" }}>
              ...
              <select value={selectedSupId} onChange={e => setSelectedSupId(e.target.value)} style={{ width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #cbd5e1", outline: "none", marginBottom: "12px", fontWeight: "600", color: "#334155", background: "#f8fafc", cursor: "pointer", transition: "border 0.2s" }}>
                <option value="">-- Chọn Nhà Cung Cấp --</option>
                {suppliers.map(s => <option key={s.id} value={s.id}>{s.name} - {s.phone} (Đang nợ: {(s.debt || 0).toLocaleString()}đ)</option>)}
              </select>
              <textarea placeholder="Ghi chú phiếu nhập (Không bắt buộc)..." value={note} onChange={e => setNote(e.target.value)} style={{ width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #cbd5e1", outline: "none", height: "70px", resize: "none", color: "#334155", background: "#f8fafc" }} />
            </div>
            ...
            <div style={{ background: "#ffffff", padding: "20px", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.02)", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ position: "relative", flex: 1 }}>
                ...
                <input type="text" placeholder="Nhập Tên hoặc Mã sản phẩm..." value={searchProd} onChange={e => setSearchProd(e.target.value)} style={{ width: "100%", padding: "12px 12px 12px 38px", borderRadius: "8px", border: "1px solid #cbd5e1", outline: "none", color: "#334155", fontWeight: "500", boxShadow: "inset 0 1px 2px rgba(0,0,0,0.02)" }} />
                ...
                {poItems.length === 0 ? <tr><td colSpan={5} style={{ textAlign: "center", padding: "40px 20px", color: "#94a3b8", fontStyle: "italic" }}>Chưa có sản phẩm nào trong phiếu nhập. Hãy tìm kiếm ở trên.</td></tr> : poItems.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: "1px solid #f1f5f9", background: idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                    ...
                    <td style={{ padding: "12px 14px", textAlign: "center" }}>
                      <input type="number" min="1" value={item.qty || ""} onChange={e => updateItem(item.product.id, 'qty', e.target.value)} style={{ width: "70px", padding: "8px", textAlign: "center", border: "1px solid #cbd5e1", borderRadius: "6px", fontWeight: "600", color: "#0f172a", outline: "none" }} />
                    </td>
                    <td style={{ padding: "12px 14px", textAlign: "right" }}>
                      <input type="text" value={item.importPrice === 0 ? "" : item.importPrice.toLocaleString()} onChange={e => updateItem(item.product.id, 'importPrice', e.target.value)} style={{ width: "110px", padding: "8px", textAlign: "right", border: "1px solid #cbd5e1", borderRadius: "6px", fontWeight: "600", color: "#0f172a", outline: "none" }} />
                    </td>
                    <td style={{ padding: "12px 14px", textAlign: "right", fontWeight: "800", color: "#0f172a" }}>{(item.qty * item.importPrice).toLocaleString()}đ</td>
                    <td style={{ padding: "12px 14px", textAlign: "center" }}>
                      <button onClick={() => removeItem(item.product.id)} style={{ background: "#fee2e2", border: "none", width: "28px", height: "28px", borderRadius: "6px", color: "#ef4444", cursor: "pointer", display: "flex", justifyContent: "center", alignItems: "center" }}>
        ...
        <div style={{ background: "#ffffff", padding: "20px 24px", borderTop: "1px solid #e2e8f0", display: "flex", justifyContent: "flex-end" }}>
          <div style={{ width: "400px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", fontSize: "14px", color: "#475569" }}>
              ...
              <input type="text" placeholder="0" value={paidAmountStr} onChange={e => { const val = e.target.value.replace(/[^0-9]/g, ''); setPaidAmountStr(val ? parseInt(val).toLocaleString() : "") }} style={{ width: "140px", padding: "8px 12px", textAlign: "right", border: "2px solid #e2e8f0", borderRadius: "8px", fontWeight: "800", color: "#10b981", fontSize: "15px", outline: "none" }} />
            </div>
            ...
            <button disabled={loading} onClick={onSubmit} style={{ width: "100%", padding: "14px", background: "linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)", color: "#fff", border: "none", borderRadius: "10px", fontWeight: "800", cursor: loading ? "not-allowed" : "pointer", fontSize: "15px", marginTop: "20px", boxShadow: "0 4px 12px rgba(37,99,235,0.3)", display: "flex", justifyContent: "center", alignItems: "center", gap: "8px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
