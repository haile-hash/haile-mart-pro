          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", width: "100%" }}>
            ...
            <div style={{ background: "#ffffff", padding: "20px", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.02)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
                ...
                <h3 style={{ margin: 0, fontSize: "15px", color: "#0f172a", fontWeight: "700" }}>Thông tin Nhà cung cấp</h3>
              </div>
              <select value={selectedSupId} onChange={e => setSelectedSupId(e.target.value)} style={{ boxSizing: "border-box", width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #cbd5e1", outline: "none", marginBottom: "12px", fontWeight: "600", color: "#334155", background: "#f8fafc", cursor: "pointer", fontSize: "14px", fontFamily: "'Inter', sans-serif" }}>
                <option value="">-- Chọn Nhà Cung Cấp --</option>
                {suppliers.map(s => <option key={s.id} value={s.id}>{s.name} - {s.phone} (Nợ: {(s.debt || 0).toLocaleString()}đ)</option>)}
              </select>
              <textarea placeholder="Ghi chú phiếu nhập (Không bắt buộc)..." value={note} onChange={e => setNote(e.target.value)} style={{ boxSizing: "border-box", width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #cbd5e1", outline: "none", height: "70px", resize: "none", color: "#334155", background: "#f8fafc", fontSize: "14px", fontFamily: "'Inter', sans-serif" }} />
            </div>
            ...
            <div style={{ background: "#ffffff", padding: "20px", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.02)", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
                ...
                <h3 style={{ margin: 0, fontSize: "15px", color: "#0f172a", fontWeight: "700" }}>Tìm & Thêm sản phẩm</h3>
              </div>
              <div style={{ position: "relative", flex: 1 }}>
                <div style={{ position: "absolute", left: "12px", top: "12px", color: "#94a3b8" }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                </div>
                <input type="text" placeholder="Nhập Tên hoặc Mã sản phẩm..." value={searchProd} onChange={e => setSearchProd(e.target.value)} style={{ boxSizing: "border-box", width: "100%", padding: "12px 12px 12px 38px", borderRadius: "8px", border: "1px solid #cbd5e1", outline: "none", color: "#334155", fontWeight: "500", fontSize: "14px", fontFamily: "'Inter', sans-serif" }} />
          ...
          <div style={{ background: "#ffffff", borderRadius: "12px", border: "1px solid #e2e8f0", overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.02)" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
              <thead style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
                <tr>
                  <th style={{ padding: "14px", textAlign: "left", color: "#475569", fontWeight: "700", textTransform: "uppercase", fontSize: "12px", letterSpacing: "0.5px" }}>Sản phẩm</th>
                  <th style={{ padding: "14px", textAlign: "center", color: "#475569", fontWeight: "700", textTransform: "uppercase", fontSize: "12px", letterSpacing: "0.5px", width: "120px" }}>SL Nhập</th>
                  <th style={{ padding: "14px", textAlign: "right", color: "#475569", fontWeight: "700", textTransform: "uppercase", fontSize: "12px", letterSpacing: "0.5px", width: "150px" }}>Giá Vốn (Nhập)</th>
                  <th style={{ padding: "14px", textAlign: "right", color: "#475569", fontWeight: "700", textTransform: "uppercase", fontSize: "12px", letterSpacing: "0.5px", width: "150px" }}>Thành Tiền</th>
                  <th style={{ padding: "14px", textAlign: "center", color: "#475569", width: "50px" }}></th>
                </tr>
              </thead>
              <tbody>
                {poItems.length === 0 ? <tr><td colSpan={5} style={{ textAlign: "center", padding: "40px 20px", color: "#94a3b8", fontStyle: "italic", fontSize: "14px" }}>Chưa có sản phẩm nào trong phiếu nhập. Hãy tìm kiếm ở trên.</td></tr> : poItems.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: "1px solid #f1f5f9", background: idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                    ...
                    <td style={{ padding: "12px 14px", textAlign: "center" }}>
                      <input type="number" min="1" value={item.qty || ""} onChange={e => updateItem(item.product.id, 'qty', e.target.value)} style={{ boxSizing: "border-box", width: "80px", padding: "8px", textAlign: "center", border: "1px solid #cbd5e1", borderRadius: "6px", fontWeight: "600", color: "#0f172a", outline: "none", fontFamily: "'Inter', sans-serif" }} />
                    </td>
                    <td style={{ padding: "12px 14px", textAlign: "right" }}>
                      <input type="text" value={item.importPrice === 0 ? "" : item.importPrice.toLocaleString()} onChange={e => updateItem(item.product.id, 'importPrice', e.target.value)} style={{ boxSizing: "border-box", width: "120px", padding: "8px", textAlign: "right", border: "1px solid #cbd5e1", borderRadius: "6px", fontWeight: "600", color: "#0f172a", outline: "none", fontFamily: "'Inter', sans-serif" }} />
                    </td>
                    <td style={{ padding: "12px 14px", textAlign: "right", fontWeight: "800", color: "#0f172a" }}>{(item.qty * item.importPrice).toLocaleString()}đ</td>
                    <td style={{ padding: "12px 14px", textAlign: "center" }}>
                      <button onClick={() => removeItem(item.product.id)} style={{ background: "#fee2e2", border: "none", width: "28px", height: "28px", borderRadius: "6px", color: "#ef4444", cursor: "pointer", display: "flex", justifyContent: "center", alignItems: "center", transition: "all 0.2s" }}>
        ...
        <div style={{ background: "#ffffff", padding: "20px 24px", borderTop: "1px solid #e2e8f0", display: "flex", justifyContent: "flex-end", borderRadius: "0 0 16px 16px" }}>
          <div style={{ width: "400px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", fontSize: "14px", color: "#475569" }}>
              ...
              <input type="text" placeholder="0" value={paidAmountStr} onChange={e => { const val = e.target.value.replace(/[^0-9]/g, ''); setPaidAmountStr(val ? parseInt(val).toLocaleString() : "") }} style={{ boxSizing: "border-box", width: "150px", padding: "8px 12px", textAlign: "right", border: "2px solid #e2e8f0", borderRadius: "8px", fontWeight: "800", color: "#10b981", fontSize: "15px", outline: "none", fontFamily: "'Inter', sans-serif" }} />
            </div>
            ...
            <button disabled={loading} onClick={onSubmit} style={{ width: "100%", padding: "14px", background: "linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)", color: "#fff", border: "none", borderRadius: "10px", fontWeight: "800", cursor: loading ? "not-allowed" : "pointer", fontSize: "15px", marginTop: "20px", display: "flex", justifyContent: "center", alignItems: "center", gap: "8px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
