        ...
        <div style={{ padding: '24px', overflowY: 'auto' }}>
          ...
          {checkoutStep === 2 && (
            <div style={{ display: 'flex', gap: '24px', alignItems: 'stretch' }}>
              ...
              {(customersData?.[custPhone]?.wallet || 0) > 0 && (<label style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 16px", borderRadius: "8px", cursor: "pointer", border: "1px solid #cbd5e1", background: '#fff', margin: 0 }}><input type="checkbox" checked={useWallet} onChange={e => setUseWallet(e.target.checked)} style={{ width: '18px', height: '18px', accentColor: '#ea580c' }} /><span style={{ fontSize: "14px", fontWeight: "600", color: "#334155" }}>Sử dụng Ví VIP: <span style={{ color: "#ea580c", fontWeight: 'bold' }}>{(customersData?.[custPhone]?.wallet || 0).toLocaleString()}đ</span></span></label>)}
                <div>
