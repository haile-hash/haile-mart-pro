...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  useEffect(() => {
    if (isCheckoutOpen && checkoutStep === 1) {
      setTimeout(() => {
        document.getElementById('co-customer-input')?.focus();
      }, 100);
    }
        ...
        <div className="checkout-body" style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="co-group" style={{ margin: 0 }}>
                ...
                <div className="co-input-wrapper">
                  ...
                  <input 
                    id="co-customer-input" 
                    className="co-input" 
                    placeholder="Nhập SĐT hoặc quét thẻ..." 
                    value={customerInput || ''} 
              ...
              <div className="co-group" style={{ margin: 0 }}>
                ...
                <div className="co-input-wrapper">
                  ...
                  <input 
                    className="co-input" 
                    placeholder="Tên khách hàng thân thiết..." 
                    value={custName || ''} 
              ...
              <div className="co-group" style={{ margin: 0 }}>
                ...
                <div className="co-input-wrapper">
                  ...
                  <input 
                    className="co-input" 
                    placeholder="Số nhà, tên đường (Nếu giao tận nơi)..." 
                    value={custAddress || ''} 
              ...
              <div className="co-summary-box" style={{ marginTop: '10px', marginBottom: 0 }}>
                ...
                <div className="co-summary-price" style={{ fontSize: '36px' }}>
                  {(finalToPay || 0).toLocaleString()}đ
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ display: 'flex', gap: '24px', alignItems: 'stretch' }}>
              ...
              <div style={{ flex: '0 0 260px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div className="co-summary-box" style={{ margin: 0, padding: '20px 15px' }}>
                  ...
                  <div className="co-summary-price" style={{ fontSize: '32px', color: '#2563eb' }}>
                    {(finalToPay || 0).toLocaleString()}đ
                  </div>
                </div>

                <div style={{ 
                  flex: 1, background: '#f8fafc', padding: '16px', 
                  border: '1px dashed #3b82f6', borderRadius: '12px', 
                  ...
                  <img 
                    src={vietQrUrl} 
                    alt="VietQR" 
                    style={{ width: '160px', height: '160px', margin: '0 auto', background: '#fff', padding: '6px', border: '1px solid #e2e8f0', borderRadius: '8px' }} 
                  />
                  ...
                  <div style={{ 
                    marginTop: '8px', fontSize: '11px', color: '#059669', 
                    fontWeight: 'bold', display: 'inline-block', background: '#dcfce7', 
              ...
              <div style={{ flex: '1', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {/* BẢO VỆ BẰNG OPTIONAL CHAINING */}
                {customers?.[custPhone]?.wallet > 0 && (
                  <label style={{ 
                    display: "flex", alignItems: "center", gap: "10px", 
                    padding: "12px 16px", borderRadius: "8px", cursor: "pointer", 
                    border: "1px solid #cbd5e1", background: '#fff', margin: 0 
                  }}>
                    <input 
                      type="checkbox" 
                      checked={useWallet} 
                    ...
                    <span style={{ fontSize: "14px", fontWeight: "600", color: "#334155" }}>
                      Sử dụng Ví VIP: <span style={{ color: "#ea580c", fontWeight: 'bold' }}>{customers[custPhone].wallet.toLocaleString()}đ</span>
                    </span>
                  </label>
                )}
                  ...
                  <div style={{ height: '22px', marginTop: '6px' }}>
                    {isValidGivenNum && givenNum > finalToPay && (
                      <span style={{ fontSize: '14px', color: '#10b981', fontWeight: '700' }}>
                        ↳ Thừa trả khách: {(givenNum - finalToPay).toLocaleString()}đ
                      </span>
                    )}
                    {isValidGivenNum && givenNum < finalToPay && (
                      <span style={{ fontSize: '14px', color: '#ea580c', fontWeight: '700' }}>
                        ↳ Còn thiếu (Quét QR nốt): {(finalToPay - givenNum).toLocaleString()}đ
                      </span>
                    )}
                  </div>
                </div>
                ...
                <button 
                  onClick={() => {
                    ...
                    if(!isValidGivenNum || givenNum >= finalToPay) {
                      alert("Thanh toán KẾT HỢP chỉ dùng khi khách đưa một phần tiền mặt (nhỏ hơn tổng bill), phần còn thiếu sẽ quét QR.\n\nVui lòng nhập lại số tiền mặt khách đưa."); 
                      return;
                    }
                    confirmCheckout('KẾT HỢP');
                  }} 
                  disabled={loading} 
                  className="btn-method orange" 
                  style={{ padding: '16px', fontSize: '15px' }}
                >
          ...
          {checkoutStep === 3 && (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <h2 style={{ color: "#059669", marginBottom: "8px", fontSize: '24px', fontWeight: '900' }}>
                CHỐT ĐƠN THÀNH CÔNG!
              </h2>
              <p style={{ color: "#64748b", fontSize: "15px", marginBottom: "24px" }}>
                Chọn mẫu in để xuất cho khách hàng.
              </p>
