...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  finalToPay, customers, bankBin, bankAcc, bankNameStr, loading,
  ...
  // LẤY TÊN CỬA HÀNG ĐỘNG ĐỂ TRUYỀN VÀO VIETQR (THAY THẾ HẢI LÊ MART)
        ...
        <div className="checkout-body" style={{ padding: '24px' }}>
          {checkoutStep === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="co-group" style={{ margin: 0 }}><label className="co-label">Khách hàng</label><div className="co-input-wrapper"><div className="co-icon-box">📞</div><input id="co-customer-input" className="co-input" placeholder="Nhập SĐT hoặc quét thẻ..." value={customerInput || ''} onChange={handleCustomerInputChange} /><button type="button" className="co-btn-scan" onClick={() => setScannerMode('customer')}>Quét</button></div></div>
              <div className="co-group" style={{ margin: 0 }}><label className="co-label">Tên khách hàng</label><div className="co-input-wrapper"><div className="co-icon-box">👤</div><input className="co-input" placeholder="Tên khách hàng thân thiết..." value={custName || ''} onChange={(e) => setCustName(e.target.value)} /></div></div>
              <div className="co-group" style={{ margin: 0 }}><label className="co-label">Ghi chú giao hàng</label><div className="co-input-wrapper"><div className="co-icon-box">📍</div><input className="co-input" placeholder="Số nhà, tên đường (Nếu giao tận nơi)..." value={custAddress || ''} onChange={(e) => setCustAddress(e.target.value)} /></div></div>
              <div className="co-group" style={{ marginTop: '10px', borderTop: '1px dashed #e2e8f0', paddingTop: '20px' }}><label className="co-label" style={{ color: '#059669' }}>Mã Voucher giảm giá</label><div className="co-input-wrapper"><div className="co-icon-box" style={{ color: '#10b981' }}>🎫</div><input className="co-input" placeholder="Nhập mã + Enter..." value={voucherInput || ''} onChange={(e) => setVoucherInput(e.target.value)} onKeyDown={handleVoucherSubmit} /><button type="button" className="co-btn-scan" onClick={() => setScannerMode('voucher')}>Quét</button></div></div>
              <div className="co-summary-box" style={{ marginTop: '10px', marginBottom: 0 }}><div className="co-summary-title">TỔNG KHÁCH PHẢI TRẢ</div><div className="co-summary-price" style={{ fontSize: '36px' }}>{(finalToPay || 0).toLocaleString()}đ</div></div>
            </div>
          )}
          ...
          {checkoutStep === 2 && (
            <div style={{ display: 'flex', gap: '24px', alignItems: 'stretch' }}>
              <div style={{ flex: '0 0 260px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div className="co-summary-box" style={{ margin: 0, padding: '20px 15px' }}><div className="co-summary-title" style={{ fontSize: '13px', color: '#1d4ed8' }}>PHẢI THU KHÁCH</div><div className="co-summary-price" style={{ fontSize: '32px', color: '#2563eb' }}>{(finalToPay || 0).toLocaleString()}đ</div></div>
                <div style={{ flex: 1, background: '#f8fafc', padding: '16px', border: '1px dashed #3b82f6', borderRadius: '12px', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  ...
                  <div style={{ fontSize: '13px', marginTop: '12px', lineHeight: '1.5' }}><strong style={{ fontSize: '14px', color: '#0f172a' }}>{bankNameStr || 'Mặc định'}</strong><br/>STK: <span style={{ color: '#2563eb', fontWeight: '900', fontSize: '14px' }}>{bankAcc || 'Chưa cài đặt'}</span></div>
              ...
              <div style={{ flex: '1', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div>
                  ...
                  <div className="co-input-wrapper" style={{ height: '48px' }}><div className="co-icon-box" style={{ padding: '0 16px', fontSize: '18px' }}>💵</div><input type="number" className="co-input" placeholder="Nhập số tiền mặt..." value={customerGiven || ''} onChange={(e) => setCustomerGiven(e.target.value)} style={{ padding: '10px 14px', fontSize: '16px', fontWeight: 'bold' }} /></div>
