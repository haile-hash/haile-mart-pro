...
export const ScannerModal: React.FC<ScannerModalProps> = ({ product, barcodeCount, setBarcodeCount, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 23, 42, 0.75)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 999999 }}>
      ...
      {/* 🛑 CSS ĐẶC BIỆT CHỈ DÀNH CHO IN MÃ VẠCH (Ẩn mọi thứ khác) */}
      <style>{`
        @media print {
          body * { visibility: hidden !important; }
          .barcode-print-grid, .barcode-print-grid * { visibility: visible !important; }
          .barcode-print-grid { position: absolute; left: 0; top: 0; width: 100%; display: grid !important; grid-template-columns: repeat(4, 1fr) !important; gap: 3mm !important; }
          .barcode-print-grid > div { page-break-inside: avoid !important; }
      ...
      <div className="no-print" style={{ background: '#ffffff', borderRadius: '12px', width: '400px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
        ...
        <div>
          ...
          <input type="number" min="1" value={barcodeCount} onChange={(e) => setBarcodeCount(parseInt(e.target.value) || 1)} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '16px', outline: 'none' }} />
        </div>
      ...
      <div className="barcode-print-grid" style={{ display: 'none' }}>
        {Array.from({ length: barcodeCount }).map((_, idx) => (
          <div key={idx} style={{ width: '100%', padding: '6px', border: '1px dashed #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', boxSizing: 'border-box', background: '#fff', height: '100%' }}>
            <div style={{ fontSize: '11px', fontWeight: '900', color: '#000', marginBottom: '2px', textAlign: 'center', letterSpacing: '0.5px' }}>{storeBrandName}</div>
            <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#000', textAlign: 'center', marginBottom: '4px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{cleanName(product.name)}</div>
            <img src={barcodeUrl} alt="barcode" style={{ width: '90%', height: '40px', objectFit: 'contain' }} />
            <div style={{ fontSize: '10px', color: '#000', marginTop: '4px', textAlign: 'center', fontFamily: 'monospace' }}>{product.product_code}</div>
            <div style={{ fontSize: '14px', fontWeight: '900', color: '#000', marginTop: '2px' }}>{getActualPrice(product).toLocaleString()}đ</div>
          </div>
        ))}
      </div>
    </div>
