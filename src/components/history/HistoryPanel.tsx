...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({
  ...
  // BỌC GIÁP: Đảm bảo groupedHistory luôn là object
  ...
  const { visibleGrouped, totalLogs } = useMemo(() => {
    ...
    // BỌC GIÁP: Đảm bảo không bị lỗi Object.entries
  ...
  const renderTransactionItem = (log: TransactionLog) => {
    if (!log) return null; // An toàn nếu bản ghi bị hỏng
    
    // BỌC GIÁP: Đảm bảo log.type luôn là chuỗi
    ...
    if (safeType === "BÁN" || safeType === "GHI NỢ") {
      ...
      const existingRefunds = allLogs.filter(h => {
        ...
        return h.type === 'TRẢ HÀNG' && 
               Number(h.id) > Number(log.id) && 
               safeLogName !== "" && 
               safeHName.includes(safeLogName);
      });
    ...
    return (
      <div 
        key={log.id} 
        style={{ 
          flexShrink: 0, padding: "12px", borderBottom: "1px solid #f1f5f9", 
          display: "flex", flexDirection: "column", gap: "6px", 
          background: isRefund ? "#fff5f5" : "transparent",
          opacity: (safeType === "BÁN" || safeType === "GHI NỢ") && remainingQtyToRefund <= 0 ? 0.6 : 1 
        }}
      >
      ...
      <div style={{ display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
        <div style={{ flex: 1, position: "relative" }}>
          <input
            ...
            placeholder="Tìm giao dịch..." value={logSearchTerm || ""} onChange={(e) => setLogSearchTerm(e.target.value)}
          />
        ...
        <select
          ...
          value={logTypeFilter || "Tất cả"} onChange={(e) => setLogTypeFilter(e.target.value)}
        >
        ...
        <button
          onClick={exportToCSV}
          style={{ padding: "0 12px", height: "40px", background: "linear-gradient(135deg, #10b981 0%, #059669 100%)", color: "white", border: "none", borderRadius: "8px", fontWeight: "bold", fontSize: "13px", cursor: "pointer", display: "flex", alignItems: "center", gap: "4px", boxShadow: "0 2px 6px rgba(16,185,129,0.2)", whiteSpace: "nowrap" }}
        ><span role="img" aria-label="excel">📊</span> EXCEL</button>
      </div>
      ...
      <div style={{ maxHeight: "400px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "10px", paddingRight: "2px" }}>
            ...
            return (
              <div key={dateStr} style={{ flexShrink: 0, display: "flex", flexDirection: "column", border: "1px solid #e2e8f0", borderRadius: "8px", background: "#ffffff", overflow: "hidden" }}>
                <div
                  ...
                  style={{ padding: "12px", background: "#f8fafc", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", borderBottom: isExpanded ? "1px solid #e2e8f0" : "none", userSelect: "none" }}
                >
        ...
        {totalLogs > visibleCount && (
          <button
            ...
            style={{ marginTop: "10px", padding: "12px", background: "#f1f5f9", border: "1px dashed #cbd5e1", borderRadius: "8px", color: "#3b82f6", fontWeight: "bold", cursor: "pointer", transition: "all 0.2s" }}
            onMouseOver={(e) => (e.currentTarget.style.background = "#e2e8f0")} onMouseOut={(e) => (e.currentTarget.style.background = "#f1f5f9")}
          >⏬ Xem thêm 50 giao dịch (Còn {totalLogs - visibleCount})</button>
        )}
      </div>
    </div>
  );
