...
export const PrintManager: React.FC<PrintManagerProps> = ({
  ...
  // Tự động lấy thông tin cửa hàng từ LocalStorage (Đề phòng App.tsx quên truyền xuống)
  useEffect(() => {
    try {
  ...
  // =====================================================================
  // CSS QUAN TRỌNG: ÉP HIỂN THỊ KHI IN & ẨN MỌI THỨ KHÁC
  // =====================================================================
  const PrintStyles = () => (
    <style>{`
      @media print {
  ...
  // =====================================================================
  // HÀM BỔ TRỢ: RENDER THÔNG TIN THANH TOÁN (TIỀN MẶT, NỢ, CHUYỂN KHOẢN...)
  // =====================================================================
  const renderPaymentDetails = (order: any, isA4: boolean) => {
    ...
    return (
      <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: isA4 ? '2px dashed #cbd5e1' : '1px dashed #000' }}>
        ...
        {!isRefund && (order.paymentMethod === 'TIỀN MẶT' || order.paymentMethod === 'KẾT HỢP') && (
          <div style={rowStyle}>
            <span>Khách đưa:</span>
            <span style={{ fontWeight: 'bold' }}>{given.toLocaleString()}đ</span>
          </div>
        )}

        {!isRefund && order.paymentMethod === 'TIỀN MẶT' && change > 0 && (
          <div style={rowStyle}>
        ...
        {order.debtAmount > 0 && (
          <div style={{ ...rowStyle, color: isA4 ? '#ef4444' : '#000', fontWeight: 'bold' }}>
            ...
            <span>{Math.round(order.debtAmount).toLocaleString()}đ</span>
          </div>
        )}
      </div>
    );
        ...
        <div style={{ width: '78mm', textAlign: 'left', fontFamily: 'Arial, sans-serif', color: '#000', padding: '10px 15px', boxSizing: 'border-box' }}>
          ...
          <div style={{ borderTop: '2px dashed #000', borderBottom: '2px dashed #000', padding: '10px 0', marginBottom: '15px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr>
                  <th style={{ padding: '0 0 8px 0', textAlign: 'left', borderBottom: '1px solid #000', width: '50%' }}>Sản phẩm</th>
                  <th style={{ padding: '0 0 8px 0', textAlign: 'center', borderBottom: '1px solid #000', width: '15%' }}>SL</th>
                  <th style={{ padding: '0 0 8px 0', textAlign: 'right', borderBottom: '1px solid #000', width: '35%' }}>Thành tiền</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={idx}>
                      <td style={{ paddingTop: '8px', paddingBottom: '8px' }}>
                        <div style={{ fontWeight: 'bold' }}>{cleanProductName}</div>
                        <div style={{ fontSize: '12px', color: '#555' }}>{price.toLocaleString()}đ</div>
                      </td>
          ...
          <div style={{ fontSize: '14px', lineHeight: '1.6' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '18px', fontWeight: '900', margin: '10px 0', borderTop: '2px solid #000', paddingTop: '10px' }}>
               <span>TỔNG CỘNG:</span>
          ...
          <div style={{ textAlign: 'center', marginTop: '30px', fontSize: '13px', fontStyle: 'italic' }}>
      ...
      <div className="force-print-zone" style={{ width: '100%', backgroundColor: '#fff' }}>
        ...
        <div style={{ width: '100%', maxWidth: '210mm', margin: '0 auto', padding: '15mm 20mm', fontFamily: '"Times New Roman", Times, serif', color: '#000', boxSizing: 'border-box' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px', fontSize: '15px' }}>
            <thead>
              <tr style={{ backgroundColor: '#f1f5f9' }}>
                <th style={{ border: '1px solid #000', padding: '10px' }}>STT</th>
                <th style={{ border: '1px solid #000', padding: '10px' }}>Tên hàng hóa</th>
                <th style={{ border: '1px solid #000', padding: '10px' }}>SL</th>
                <th style={{ border: '1px solid #000', padding: '10px' }}>Đơn giá</th>
                <th style={{ border: '1px solid #000', padding: '10px' }}>Thành tiền</th>
              </tr>
            </thead>
            <tbody>
              {(lastOrder.cart || []).map((item: any, idx: number) => {
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '40px' }}>
            <div style={{ width: '450px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '15px' }}>
                <span>Cộng tiền hàng:</span><span style={{ fontWeight: 'bold' }}>{Math.round(lastOrder.subTotal).toLocaleString()}đ</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '15px' }}>
                <span>Thuế VAT:</span><span style={{ fontWeight: 'bold' }}>{Math.round(lastOrder.vatTotal).toLocaleString()}đ</span>
              </div>
              {lastOrder.discount > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '15px' }}>
                  <span>Chiết khấu / Giảm giá:</span><span style={{ fontWeight: 'bold' }}>-{Math.round(lastOrder.discount).toLocaleString()}đ</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', fontSize: '20px', fontWeight: 'bold', borderTop: '2px solid #000', borderBottom: '2px solid #000', marginTop: '10px' }}>
                <span>TỔNG THANH TOÁN:</span>
                <span>{Math.round(lastOrder.debtAmount > 0 ? lastOrder.debtAmount : lastOrder.finalTotal).toLocaleString()}đ</span>
              </div>
              <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px' }}>
                {renderPaymentDetails(lastOrder, true)}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
            ...
            <div style={{ textAlign: 'center', width: '250px' }}>
              <p style={{ margin: '0 0 5px 0', fontSize: '16px', fontWeight: 'bold', textTransform: 'uppercase' }}>{currentStore?.store_name || "ĐẠI DIỆN CỬA HÀNG"}</p>
              <p style={{ margin: '0 0 100px 0', fontSize: '14px', fontStyle: 'italic' }}>(Đại diện ký, đóng dấu)</p>
            </div>
          </div>
        </div>
      </div>
  ...
  // CÁC CHẾ ĐỘ IN KHÁC GIỮ NGUYÊN (Mã vạch, PO, Customer Card...)
  // ... (Bạn giữ nguyên phần code in mã vạch và PO cũ ở bên dưới nếu cần)
