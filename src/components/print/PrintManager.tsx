...
export const PrintManager: React.FC<PrintManagerProps> = ({
  ...
  const renderOrderInvoice = (isA4: boolean) => {
    if (!lastOrder) return null; // Chặn render nếu mất data đơn hàng
  ...
  // =====================================================================
  if (['receipt', 'bill', 'pos'].includes(safeMode)) {
    return renderOrderInvoice(false);
  }
  
  if (['invoice_a4', 'a4', 'invoice'].includes(safeMode)) {
  ...
  if (['customer_card', 'card'].includes(safeMode)) {
...
// =====================================================================
  ...
  const renderPO = () => {
    ...
    const isReceipt = safeMode === 'po_receipt'; // Phân biệt in Đặt hàng hay in Nhận hàng
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "25px", fontSize: "14px" }}>
          ...
          <tbody>
            {po.items?.map((item: any, idx: number) => {
              // Nếu in phiếu nhận hàng, chỉ tính số lượng thực tế (đã trừ hàng hỏng)
              const actualQty = isReceipt ? Math.max(0, item.qty - (item.damagedQty || 0)) : item.qty;
              const itemTotal = actualQty * item.importPrice;
              ...
              return (
                <tr key={idx}>
                  ...
                  <td style={{ border: "1px solid #000", padding: "10px", textAlign: "right" }}>{Number(item.importPrice).toLocaleString()}đ</td>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <tbody>
              <tr>
                ...
                <td style={{ padding: "6px 0", textAlign: "right", fontWeight: "bold", fontSize: "16px" }}>{Number(po.total_amount).toLocaleString()}đ</td>
              </tr>
              <tr>
                <td style={{ padding: "6px 0", textAlign: "left" }}>Đã thanh toán trước:</td>
                <td style={{ padding: "6px 0", textAlign: "right" }}>{Number(po.paid_amount).toLocaleString()}đ</td>
              </tr>
              <tr style={{ borderTop: "2px solid #000" }}>
                <td style={{ padding: "8px 0", textAlign: "left", fontWeight: "bold", color: "#b91c1c" }}>CÒN NỢ NCC:</td>
                <td style={{ padding: "8px 0", textAlign: "right", fontWeight: "bold", color: "#b91c1c" }}>{Number(po.debt_amount).toLocaleString()}đ</td>
              </tr>
            </tbody>
          </table>
        </div>
