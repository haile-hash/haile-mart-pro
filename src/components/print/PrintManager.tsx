...
export const PrintManager: React.FC<PrintManagerProps> = ({
  ...
  const renderPaymentDetails = (order: any, cDetail: any, isA4: boolean) => {
    ...
    const rowStyle = isA4 ? { display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontSize: '14px' } : { display: 'flex', justifyContent: 'space-between', padding: '2px 0', fontSize: '12px' };
      ...
      <div className="print-only-zone" style={{ width: '100%', textAlign: 'center', backgroundColor: '#fff' }}>
        <div style={{ display: 'inline-block', width: '78mm', margin: '0 auto', textAlign: 'left', fontFamily: 'Arial, sans-serif', color: '#000', padding: '10px 0', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px', padding: '0 10px', fontSize: '11px', fontWeight: 'bold' }}>
            <div style={{ textAlign: 'center' }}>Khách hàng<br/><span style={{ fontSize: '9px', fontWeight: 'normal', fontStyle: 'italic' }}>(Ký, ghi rõ)</span></div>
            <div style={{ textAlign: 'center' }}>Người bán<br/><span style={{ fontSize: '9px', fontWeight: 'normal', fontStyle: 'italic' }}>(Ký, ghi rõ)</span></div>
          </div>
  ...
  if (printMode === 'po_order' || printMode === 'po_receipt' || printMode === 'po_return') {
    ...
    printPOData.items?.forEach((item: any) => {
      let q = item.qty;
      if (isReceipt) q = item.qty - (item.damagedQty || 0); 
      else if (isReturn) q = item.damagedQty || 0;          
      if (q > 0) actualTotal += q * item.importPrice;
    });

    const paidAmount = Number(printPOData.paid_amount || 0);
    const debtAmount = actualTotal - paidAmount; 
